...
| 9 | `docs/test-user-roles.md` | **`profiles.role`** + **`has_active_subscription`** — tier testing SQL recipes |
| 10 | `supabase/*.sql` | Schema, RLS, triggers; read headers/comments when touching the DB |
...
| **Freemium / subscriptions / entitlements** (tiers, paywalls, Stripe) | **`docs/access-tiers.md`** (shipped tiers) + **`docs/entitlements-matrix.md`** (multi-product catalog + capability matrix) + `docs/social-feed-roadmap.md` (Freemium section) + backlog + `docs/frontend-architecture.md` when client gating changes |
...
| **Lounge bots (Scott Share / odds, wire, X editorial)** | **`docs/lounge-bot-sports-odds.md`**, **`docs/syndicate-cfb-weekly-runbook.md`** (CFB board + four-desk weekly SOP … internal), **`docs/syndicate-ufc-desk-contract.md`** (UFC 4-desk rebuild … locked, do not code until independence test is a real check), **`docs/syndicate-lane-b-handicappers.md`** (external weighted handicappers desk / intake allowlist … **`data/syndicate/lane-b-intake-allowlist.csv`**), **`docs/lounge-bot-market-news.md`**, **`docs/lounge-bot-crypto-news.md`**, **`docs/lounge-bot-editorial-queue.md`**, **`docs/lounge-bot-farm-ingest.md`** (external worker door, **`lounge-bot-ingest`**); portal **`/?tab=bots`** (**`BotReplyOnPostPanel`**, **`BotProfileEditor`**, **`BotComposeImagePicker`**, X **Transform a post**); Edge **`lounge-odds-ingest`**, **`lounge-odds-poll`**, **`lounge-news-poll`**, **`lounge-x-ingest`**, **`lounge-bot-ingest`**; migrations through **`20260707000000`** on test + prod (**`schema_migrations`** tracked Jul 7); X manual transform works without bearer |
