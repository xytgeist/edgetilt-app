...
- Lounge dock chrome + drawers: `src/utils/loungeDockChrome.js`, **`LoungeDockFooterBar.jsx`**, **`LoungeDockSlidePanels.jsx`** (search / notifications / chat: swipe dismiss **left or right**; deferred `setPointerCapture` until horizontal intent; list regions **`touch-pan-y`** so horizontal pans are not stolen by the browser; **`SocialFeed.jsx`** feed bottom inset + slide **`bottomReservePx`**).
