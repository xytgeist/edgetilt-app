...
- Lounge dock chrome + drawers: `src/utils/loungeDockChrome.js`, **`LoungeDockFooterBar.jsx`**, **`LoungeDockSlidePanels.jsx`** (search / notifications / chat: **full-screen** column `max-w-2xl`; **feed-matching title bar** + scroll-linked hide/show via **`loungeTitleRevealScroll.js`**; **dock `reveal={1}`** on those screens while the feed dock stays scroll-linked; **search** uses full **`LoungePostArticle`** cards + **`loungePostInteractionScore`** sort, **`LoungeFeedVideoAutoplayProvider`**, `postCardProps` from **`SocialFeed.jsx`**; swipe dismiss left/right; main title/dock hidden while panel open).
