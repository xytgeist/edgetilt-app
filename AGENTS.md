...
- Lounge **likes UI:** **`LoungeFlameIcon.jsx`** (poker chip + heart; liked **`#fd262d`** / **`lv-red`**); **`LoungeLikeStatContent`** fixed grid so icon does not shift when count changes; **Share** in post **⋯** menu only (removed from interaction bar). **`LoungePostInteractionBar.jsx`**, **`LoungePostRowMenu.jsx`**, **`SocialFeed.jsx`**.
