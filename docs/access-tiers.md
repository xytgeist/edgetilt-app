...
| **W-2G Scanner** | **Free** … single-scan OCR + per-user cloud archive (private Storage + `w2g_slips`). Hub tile unlocked; **Save to archive** requires sign-in. **Bulk import** (multi-photo OCR+save) requires **Slots Edge** (Starter) or higher. |
