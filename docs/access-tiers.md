...
| **W-2G Scanner** | **Free** … single-scan extract + per-user cloud archive (private Storage + `w2g_slips`). Hub tile unlocked; **Save to archive** requires sign-in. **EdgeiOS Take photo** uses VisionKit when the IPA has `scanDocument`. **Extract** is the same for free and Starter+: IPA uses on-device Vision, then OpenAI if the slip looks unsure; PWA / old IPA use OpenAI when signed in, else tesseract. **Play Logbook** cash out ≥ $2,000 offers Scan the W-2G (prefill date + box 1). **Bulk import** is the only Starter+ difference (IPA uses PHPicker when the binary has `pickPhotos`). |
...
**Military 25% forever:** personal links **`edgetilt.com/mil25/{CODE}`** (unique Stripe promotion codes on coupon **`9zheeC1H`**, optional secret **`STRIPE_COUPON_MILITARY`**). Logged-out visitors get Join / Sign in first, then the Subscribe picker. Subscribe cards swap Founding → Military. Checkout applies that promotion code and does **not** stack with founding or creator promos. Used codes fail closed.
...
**App Store Lifetime (2026-09-14):** list US **`$1,499.99`** for the founding window (was `$1,999.00`). StoreKit charges that sticker. Raise the list when founding ends. Offer-code path parked. Web founding **`$1,274.25`** still works via Safari.
...
| **Signup / verification** | **Continue with Phone** is Supabase SMS login. The US and Canada use the 2FA number. Other countries in the menu use the sender name EdgeTilt. A new phone account copies that number into Account info. Changing the Account info phone, while signed in, texts a code and links that number to the same account. A confirmed phone shows the Edge Pro check next to the Lounge display name. Remove is in Account info once that account has a confirmed email. We text the current phone first. The check comes off and the number can be added on another account. Adding or changing an email from Account info uses a 6-digit code. Signup, password reset, and guest claim emails stay links. Test Auth keeps SMS autoconfirm off. If it is on, that link confirms the number and never texts. Continue with Phone still texts either way. Not the Lounge verified badge. **Planned:** phone OTP → **Verified** badge via **Twilio Verify** (auth-only; no guest swap/stake SMS). Backlog **`docs/test-buildout-backlog.md` → Planned (Account & identity verification)**. Placeholder **`profiles.phone_number`** + Account info UI on test; badge not shipped. Email confirm exists at login; not used for badge today. |
