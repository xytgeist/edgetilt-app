...
| **W-2G Scanner** | **Free** … single-scan local OCR + per-user cloud archive (private Storage + `w2g_slips`). Hub tile unlocked; **Save to archive** requires sign-in. **EdgeiOS Take photo** uses VisionKit when the IPA has `scanDocument` (still free). **EdgeiOS extract** uses on-device Vision OCR for everyone on a current IPA, then OpenAI only if the slip looks unsure (signed-in, still free). **PWA AI vision extract** + **Bulk import** require **Slots Edge** (Starter) or higher (Edge **`w2g-vision-extract`**). |
