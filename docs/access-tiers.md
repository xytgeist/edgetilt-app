...
| **Signup / verification** | **Planned:** phone OTP → **Verified** badge via **Twilio Verify** (auth-only; no guest swap/stake SMS). Backlog **`docs/test-buildout-backlog.md` → Planned (Account & identity verification)**. Placeholder **`profiles.phone_number`** + Account info UI on test; badge not shipped. Email confirm exists at login; not used for badge today. |
