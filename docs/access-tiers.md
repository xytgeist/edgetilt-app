...
| **Signup / verification** | **Continue with Phone** is Supabase SMS login. The US and Canada use the 2FA number. Other countries in the menu use the sender name EdgeTilt. A new phone account copies that number into Account info. Changing the Account info phone, while signed in, texts a code and links that number to the same account. Test Auth keeps SMS autoconfirm off. If it is on, that link confirms the number and never texts. Continue with Phone still texts either way. Not the Lounge verified badge. **Planned:** phone OTP → **Verified** badge via **Twilio Verify** (auth-only; no guest swap/stake SMS). Backlog **`docs/test-buildout-backlog.md` → Planned (Account & identity verification)**. Placeholder **`profiles.phone_number`** + Account info UI on test; badge not shipped. Email confirm exists at login; not used for badge today. |
