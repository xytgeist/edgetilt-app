...
| **W-2G Scanner** | **Free** … single-scan local OCR + per-user cloud archive (private Storage + `w2g_slips`). Hub tile unlocked; **Save to archive** requires sign-in. **AI vision extract** + **Bulk import** require **Slots Edge** (Starter) or higher (Edge **`w2g-vision-extract`**). |
