...
| Featured layout | Recorder’s **pin** at Record start → custom RoomComposite `focus:<identity>`; **no pin** → recorder’s own camera. Locked for that segment (no live pin follow). Main + PiPs + EDGE brand. |
...
- **Egress template:** vanilla (no React) Vite page → **`dist/call-egress.html`** + external JS, mirrored to R2 via **`publish-call-egress-template`** / **`publish-call-egress-template-local.mjs`** (`https://media-test.lvslotpro.com/call-egress/call-egress.html` on test). Layout = **`focus:<identity>`** (pin / recorder). `CHAT_CALL_EGRESS_USE_CUSTOM=0` forces built-in `speaker`. Do not point LiveKit at the Vercel HTML.
...
- **Voice call live transcript:** after answer **and** call audio playback is unlocked, each participant streams **local mic only** to Deepgram live (`nova-3`) via a short-lived **`mint_live_stt_grant`** JWT (browser WS). PCM capture uses a dedicated AudioContext sunk to **`MediaStreamDestination`** (never `audioCtx.destination`) so STT does not fight iPhone call playback. Finals flush with **`append_live_transcript`** into `chat_calls.live_transcript` (Edge stamps `user_id` from JWT... no diarization guess). On call end (and late flush ≤90s), draft is copied onto the **`call_summary`** card as `provider: deepgram_live`. No LiveKit egress / no R2 audio for voice.
...
- **Call audio (iPhone focus):** Accept/Start sets AudioSession **`play-and-record`**; hangup resets **`playback`→`auto`**. Speaker button hidden unless real sink switching exists (no silent no-op toggle). **1:1 voice** uses `webAudioMix: false` (HTML `<audio>`); **group** (and video) keep `webAudioMix: true`. `room.startAudio()` on connect + retries; “Tap for call audio” if still blocked. True guaranteed earpiece still needs CallKit / native... web can only bias.
