...
- **Egress template:** isolated single-file Vite page **`/call-egress.html`** (`vite.call-egress.config.js` + `src/call-egress/`)... LiveKit Chrome loads `layout=focus:<id>` (pinned cam, else recorder). Starts recording as soon as Connected. Escape hatch: **`CHAT_CALL_EGRESS_USE_CUSTOM=0`** → built-in `speaker`. Test host `https://lvslotpro.com/call-egress.html`; prod `https://edgetilt.com/call-egress.html`.
