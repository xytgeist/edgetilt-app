...
- **Egress template:** vanilla (no React) Vite page → **`dist/call-egress.html`** with JS **inlined**, mirrored to R2 via **`publish-call-egress-template`** / **`publish-call-egress-template-local.mjs`** (`https://media-test.lvslotpro.com/call-egress/call-egress.html` on test). Pin layout = `layout=focus:<id>`. **Currently forced to LiveKit built-in `speaker`** in `chat-calls` until a Record→Stop smoke saves an MP4 with the vanilla template. Do not point LiveKit at the Vercel HTML.
