...
- **Call summary card (historical):** on call end, Edge inserts a durable `content_encoding = call_summary` message (`ChatCallSummaryCard`) with `link_preview.kind = call_summary` (status, media_mode, duration, participant avatars). Stays in the thread after leave/reopen. Live late-join Join bar is separate and only while the call is open. Unique index on `link_preview->>'call_id'` for call_summary. **Voice calls** also surface **View transcript** when live STT produced lines (no playable audio / no recording card).
...
- **Voice call live transcript:** after answer, each participant streams **local mic only** to Deepgram live (`nova-3`) via a short-lived **`mint_live_stt_grant`** JWT (browser WS). Finals flush with **`append_live_transcript`** into `chat_calls.live_transcript` (Edge stamps `user_id` from JWT... no diarization guess). On call end (and late flush ≤90s), draft is copied onto the **`call_summary`** card as `provider: deepgram_live`. No LiveKit egress / no R2 audio for voice.
