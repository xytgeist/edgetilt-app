...
- **Add people (in-call):** Header top-right person+ opens a follow/follower picker with search. Tap sends `invite_to_call` (VoIP + `chat_call_invite`). A DM call is promoted to a new group so the invitee can accept. Redeploy **`chat-calls`** on test. **New TestFlight not required** for the button (web chrome).
