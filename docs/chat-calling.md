...
- **Egress template:** auth-free Vite page **`/call-egress.html`** (`src/call-egress/`)... LiveKit headless Chrome loads it with `url` / `token` / `layout=focus:<id>`. Test host `https://lvslotpro.com/call-egress.html`; prod `https://edgetilt.com/call-egress.html`. Template must `console.log('START_RECORDING')` quickly (hard-start ~4s) or Stop yields **Recording failed to save** with no R2 object.
