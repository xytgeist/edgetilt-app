...
- **Egress template:** auth-free Vite page **`/call-egress.html`** (`src/call-egress/`) for pin-featured layout. **Default on test/prod right now:** LiveKit built-in **`speaker`** layout (no `customBaseUrl`) so recordings reliably land on R2. Re-enable pin template with Edge secret **`CHAT_CALL_EGRESS_USE_CUSTOM=1`** + template URL once Chrome `START_RECORDING` is proven.
