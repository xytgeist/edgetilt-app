...
- **Paid KPIs skip comps (20260916150000):** `ops_monitor_is_paid_billing()` ... admin-comp / test grants (friend Lifetime) stay entitled but do not count in snapshot Lifetime, active-by-product, webhook billing-sub expect, or roster product totals. Roster lists still show Comp rows.
...
**Poker catalog heartbeat (40800):** Production sync heartbeats **`admin_ops_job_heartbeats`**. Primary runner is **Windows Task Scheduler** on the home PC (daily 2:00 AM), not GitHub `ubuntu-latest`. Monitor stale if last success >4 days. Panel shows remaining MTTDB counts. **One scrape, two writes:** `--mirror-production` upserts test then prod from the same rows. **MTTDB scrape is home-PC only** … GitHub Actions skips it (`--skip-mttdb` / `GITHUB_ACTIONS`) because Cloudflare blocks ubuntu 100%. ClubWPT / CoinPoker / Wynn / regional still run on GHA. Amber CF note only when the home scrape actually hit Turnstile.
...
_Update log: 2026-09-16 — Paid Monitor KPIs skip comps (`20260916150000`, test + prod). Only `@kix123` counts as paid Lifetime._
