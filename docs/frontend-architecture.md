...
| Folder | Role |
| --- | --- |
| **`shell/`** | `AppShell.jsx`, `shellClasses.js` (shared Tailwind class strings for layout/buttons where reused), `index.js` barrel. **Tab layout:** Offers, Bankroll, Intel, and dashboard/team placeholders use the same **`max-w-2xl`** column + **`px-3`** inset as Lounge / `ScrollLinkedEdgeTitleBarShell` so surfaces align edge-to-edge within the shell. |
...
| **`offers/`** | Offers calendar: `OffersCalendar.jsx` (root **`max-w-2xl`** + **`px-3`** with Lounge; week landscape full-bleed unchanged), `offerStorageKeys.js`, `utils.js`, `hooks/`, `components/`, `index.js` (re-exports `OffersCalendar`). |
