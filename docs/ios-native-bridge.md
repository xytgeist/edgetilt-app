...
| WKWebView `alert` / `confirm` / `prompt` | Mac | **Native (2026-09-12):** custom `uiDelegate` now implements the JS dialog panels. Without them, `window.confirm` is a silent Cancel … W-2G delete looked dead. W-2G delete also uses AppShell in-app confirm so it works on the current IPA before a new binary. |
