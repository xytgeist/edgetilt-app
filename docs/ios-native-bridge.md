...
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit Live Activity for live slots / poker sessions. Compact Island + Lock Screen. Tap → `edgetilt://live-session?tab=bankroll` / `poker-bankroll`. Web chip hides on IPA when this reports `ok` and not `disabled`. Old IPA / PWA keep the title-bar chip. **2026-09-16 UI:** glyph tile leading, custom `TimelineView` elapsed (`m:ss` / `h:mm:ss` … not `showsHours: false`, which capped at 59s), pulse bars, `keylineTint`. New IPA required. CallKit still owns the Island during a Chat call. |
