...
- **Platform Slots Edge + Edge Pro:** IAP primary when StoreKit returns the product; **Subscribe on the web** is secondary on the Slots Edge paywall. **Edge Pro Settings unlock card (IPA):** IAP price only (`$11.49`), no web `$9.99` CTA. Restore + Terms/Privacy on the paywall. Apple-billed manage → `manageStoreSubscriptions`.
