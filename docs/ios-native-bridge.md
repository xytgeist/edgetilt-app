...
| `setNativeCallStreamFocus` | JS→native | `{ isLocalMain?: boolean, focusedIdentity?: string, quadFocus?: boolean }` | `{ ok: boolean, isLocalMain: boolean, focusedIdentity?: string, quadFocus?: boolean }` | Mac | **native** (2026-08-29, **2026-09-04**). Count-based tiles (duo / trio / quad / 5+ grid). You stay bottom-right. `focusedIdentity` is a remote id. `quadFocus` (4 people only) expands one remote full-bleed + right stack; empty / false keeps the 2×2. `isLocalMain` is ignored. |
