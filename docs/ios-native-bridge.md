...
| Universal Links (email confirm + Lounge share) | Both | **Mac 2026-09-11:** Associated Domains `applinks:edgetilt.com` + `lvslotpro.com` (+ www). `AppDelegate` / SwiftUI `NSUserActivityTypeBrowsingWeb` → `EdgePushManager.handleUniversalLink` → same WKWebView pending queue as APNs. **Mac 2026-09-23:** AASA + Swift also claim **`/lounge/p/*`**, **`/u/*`**, and SPA `/?post=` / `?u=` / `?profile=`. Share permalinks rewrite to `/?tab=home&post=` / `&u=` before load (skip OG HTML flash). Auth confirm unchanged. **Web:** AASA + Safari Smart App Banner (`apple-itunes-app` app-id **`6806401093`**, OG pages pass `app-argument`). New IPA required for path allowlist / rewrite. Gmail in-app can still ignore UL; `edgetilt://auth/confirm` remains the email fallback. |
