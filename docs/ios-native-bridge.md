...
| `scanDocument` | JS→native | `{ purpose?: string, maxPages?: number }` | `{ ok, cancelled?, unsupported?, images?: [{ mimeType, base64, width, height }] }` | Mac + web caller | **native** + **web caller** (2026-09-11, multi-page **2026-09-12**). VisionKit `VNDocumentCameraViewController`. Default 1 page, max 8. JPEG ≤2000px. W-2G **Take photo** asks for up to 8 pages. One page stays in the editor. Two or more import every page (free Take photo path, skips `scanic`). Cancel / Simulator `unsupported` / old IPA → hidden file input. PWA unchanged. |
