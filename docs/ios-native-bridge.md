...
| `getStoreProducts` | JS→native | `{ productIds: string[] }` | `{ products: Array<{ id, displayName, displayPrice, productSlug, introEligible?, introDisplayPrice?, introPeriodCount?, introPeriodUnit?, introPaymentMode? }> }` | Mac | **native** (StoreKit 2). Paywall uses intro price when eligible, else `displayPrice`. **Device smoke pending.** |
