...
| `reportIncomingCall` | JS→native | `{ callId, handle, hasVideo?, roomId?, avatarUrl? }` | `{ ok: boolean, uuid?: string, deduped?: true }` | Mac | **native** (2026-08-26). **Deduped by `callId` 2026-08-27** … see caution below. `avatarUrl` is accepted and ignored after the `68a9bed6` rollback. |
...
**Rollback (2026-08-27 night):** incoming CallKit + invite push restored to **`68a9bed6`** (last working landmark: Ryan's WhatsApp chrome message). Avatar / decline-from-Swift / background-JS-skip / observer-evict / always-on invite alert are out. Do not re-land those until CallKit is the ring again. The avatar / decline / background-skip notes below are historical.
