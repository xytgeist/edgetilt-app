...
| `setKeyboardAccessory` | JS→native | `{ visible: boolean }` | `{ ok, visible }` | Mac + web caller | **native** + **web caller** (2026-09-12). Default IPA hides the WK Done / prev-next bar. W-2G tax-field focus sets `visible: true` so the **system** accessory comes back. GIF picker / compose stay hidden. New IPA required. |
...
| W-2G extract keyboard accessory | Mac + web | **2026-09-12:** ripped the custom hover chip. Tax-field focus calls `setKeyboardAccessory({ visible: true })` for the stock WK Done / prev-next bar. Old IPA: no bar (same as before the custom chip). |
