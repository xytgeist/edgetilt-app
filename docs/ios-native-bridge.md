...
| `pickEdgeVideo` | JS→native | `{ purpose?: string }` | `{ ok, cancelled?, assetId?, previewUrl?, duration?, width?, height?, byteSize?, posterJpegBase64? }` | Mac + web caller | **native** + **web caller** (2026-09-22). `PHPicker` videos, one clip, copied into caches. `previewUrl` is `edge-video://local/<id>` (custom scheme registered in `makeConfiguration`, range reads). Poster is a JPEG base64. Cancel / old IPA / PWA → `<input type="file">`. Web crop/trim modal stays the UI. |
| `exportEdgeVideo` | JS→native | `{ assetId, startSec, endSec, cropPx?, intrinsicWidth?, intrinsicHeight? }` | `{ ok, passthrough?, assetId, sourceAssetId, byteSize, previewUrl }` | Mac + web caller | **native** (2026-09-22). Re-encodes with `AVAssetExportPreset1920x1080` only for a trim, a crop, a `.mov` / spatial / screen recording, or a file over 200MB. Other clips return `passthrough: true` and upload the picked file. `cropPx` is the modal rect in displayed pixels (y down). Progress: `edge-native-video-progress` `{ phase: 'export', progress, assetId }`. |
| `uploadEdgeVideoTus` | JS→native | `{ assetId, accessToken, supabaseUrl, anonKey }` | `{ ok, streamVideoUid, assetId }` | Mac + web caller | **native** (2026-09-22). Lounge only. Same tus create as the web client (`lounge-cf-stream-tus-create`, bearer + apikey), then one PATCH of the MP4 to Cloudflare `Location` on a background `URLSession`. Progress phase `upload`. JS passes the live Supabase access token. No Edge redeploy. |
