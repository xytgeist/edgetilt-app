...
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds?, pausedAt?, pauseElapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, overlay?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit for background / Lock Screen. Also feeds the **in-app fake Island** (`EdgeLiveBankrollIslandOverlay`) because iOS hides the system Island while EdgeTilt is foregrounded. Tap → manager tab only. New IPA required. |
| `setLiveBankrollIslandOverlay` | JS→native | `{ visible: boolean }` | `{ ok, visible }` | Mac + web caller | **native** + **web caller** (2026-09-17). Show / hide the in-app fake Island. Web sets `visible: false` on bankroll screens that already show the live session card. Overlay also auto-hides when the app resigns active (system Island takes over). **2026-09-17:** tap dispatches `edge-live-session-open` (no full reload); slots glyph is white die + black pips. `/?tab=bankroll` deep links now open Slots Bankroll. |
