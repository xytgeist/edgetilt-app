...
| `setOrientationLock` | JS→native | `{ lock: 'portrait'\|'none' }` | `{ ok, locked?, skipped?: 'ipad', unchanged? }` | Mac + web caller | **native** + **web caller** (2026-09-21). iPhone IPA only. Composer open → `portrait` (forces rotate). Last composer close → `none` so a still-sideways phone can go landscape again. iPad / old IPA / Safari no-op. New IPA required. |
