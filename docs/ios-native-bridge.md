...
| `pickPhotos` | JS→native | `{ purpose?: string, maxCount?: number }` | `{ ok, cancelled?, images?: [{ mimeType, base64, width, height }] }` | Mac + web caller | **native** + **web caller** (2026-09-11, app-wide **2026-09-12**). `PHPickerViewController`, images only. Default 12, max 40. JPEG ≤1600px. Wired on W-2G bulk, Lounge compose (feed / thread / reply / quote), chat images, profile banner/avatar, chat group photo, Offers mailer library. Cancel / old IPA / PWA → `<input type="file">`. Video attach stays on the file input. |
