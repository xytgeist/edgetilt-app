...
| `scanDocument` | JS→native | `{ purpose?: string, maxPages?: number }` | `{ ok, cancelled?, unsupported?, images?: [{ mimeType, base64, width, height }] }` | Mac + web caller | **native** + **web caller** (2026-09-11, review queue **2026-09-12**). VisionKit `VNDocumentCameraViewController`. Default 1 page, max 8. JPEG ≤2000px. W-2G **Take photo** asks for up to 8 pages. Every page opens the extract/review screen (save / download / adjust / share / remove). Multiple pages swipe in the image frame. Save or Remove keeps the screen and slides in the next remaining slip. |
