...
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds?, pausedAt?, pauseElapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit Live Activity for live slots / poker. iOS **suppresses the Island while EdgeTilt is foregrounded** (shows after background / Lock Screen). Title-bar chip stays in-app except on the bankroll screen that already shows the live card. Compact glyph + clamped `.timer`. Tap → manager tab only (no `pokerSession`). New IPA required. CallKit still owns the Island during a Chat call. |
