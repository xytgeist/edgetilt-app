...
| `reportIncomingCall` | JS→native | `{ callId, handle, hasVideo?, roomId?, avatarUrl? }` | `{ ok: boolean, uuid?: string, deduped?: true, skipped?: 'background' }` | Mac | **native** (2026-08-26). **Deduped by accepted `callId` 2026-08-27**. JS / APNs skip unless `.active` … VoIP owns background. `avatarUrl` is accepted and ignored. |
...
**Background receive (re-landed after `68a9bed6` rollback smoke):** JS / APNs `willPresent` skip `reportNewIncomingCall` unless `UIApplication.shared.applicationState == .active`. PushKit is the only reporter when Edge is backgrounded or locked. Dedupe-before-accept plus a background JS report is how a live `ringing` row produced silence, then a late pill after hangup. **Do not** re-add `reportCall(updated:)` avatar, observer evict inside PushKit, Swift `decline_call` / `leave_call` fallback, or always-on invite alert with this change.
