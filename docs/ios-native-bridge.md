...
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds?, pausedAt?, pauseElapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit Live Activity for live slots / poker. Running → play elapsed; paused poker → open pause clock. Compact: larger circular black-glyph mark (breathes while live), system `.timer` clamped to ~44pt so the Island stays tight. Tap → `edgetilt://live-session?tab=poker-bankroll` / `bankroll` only (no `pokerSession` … detail sheet stays closed). New IPA required. CallKit still owns the Island during a Chat call. |
