...
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds?, pausedAt?, pauseElapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit Live Activity for live slots / poker. Running → Island clock tracks **play** elapsed (`elapsedSeconds`). Paused poker → clock tracks **open pause** from `pausedAt` (amber, no pause glyph). Compact: circular black-glyph mark (breathes while live), system `Text(..., style: .timer)`. Tap → `edgetilt://live-session?tab=…`. New IPA required. CallKit still owns the Island during a Chat call. |
