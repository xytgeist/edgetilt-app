...
**Orientations (Mac 2026-09-19):** iPhone supports **portrait + landscape left/right** (`INFOPLIST_KEY_UISupportedInterfaceOrientations` in `ios/project.yml` / `project.pbxproj`). It was portrait-only before **`8b701c92`**. iPad already had all four. Web phone-landscape Slots + Lounge rail live on **`test` ≥ `b843f6ef`** / **`1.4.364`**. **2026-09-21:** iPhone IPA composers call **`setOrientationLock({ lock: 'portrait' })`** and unlock on close. New IPA required. Continuity: **`WAKEUP`** → **Mac Theo ACTIVE TRACK 2026-09-21**.
...
| **Helper** | **`src/utils/edgeNative.js`** … `isEdgeiOSShell()`, `readEdgeiOSShellVersion()`, `edgeNativeInvoke(method, payload)`, **`openExternalUrl(url)`** (shell → Safari; else `window.open`), **`openExternalBillingUrl(url)`** (shell → Safari; else `location.assign`), **`canScanEdgeDocument` / `scanEdgeDocument` / `filesFromNativeScanImages` / `tryAssignEdgeScannedDocuments`** (W-2G + Offers mailer camera), **`canRecognizeEdgeText` / `recognizeEdgeText`** (on-device Vision OCR), **`canPickEdgePhotos` / `pickEdgePhotos` / `tryAssignEdgePickedPhotos` / `dispatchFilesOnInput`** (W-2G bulk + Lounge / chat / profile / Offers library), **`canShareEdgeNative` / `shareEdgeNative` / `shareViaBestAvailable`** (system share sheet), **`syncEdgeLiveBankrollActivity`** (live slots / poker → Dynamic Island; in-app uses title-bar chips), **`useEdgeiOSComposerPortraitLock`** (`src/utils/edgeiOSComposerPortraitLock.js` … iPhone IPA portrait lock while a composer is open) |
...
| `bustServiceWorker` | JS→native *or* boot-only | none / `{ scope?: string }` | `{ ok: boolean, unregistered?: number, cacheKeysDeleted?: number }` | Mac (boot) | **native** (boot clear + bridge; 2026-08-23). **2026-09-15:** Simulator skips the boot `removeData` wait (iOS 27 WebKit can hang there and never `load`). Device still clears, 2s cap. Look for `EdgeWebView load` / `didStart` / `didFinish` / `didFailProvisional` in the Xcode console. |
...
| `signInWithApple` | JS→native | `{ nonce: string }` (`SHA-256` hex of the raw nonce) | `{ ok, identityToken?, authorizationCode?, user?, email?, fullName?, cancelled?, error?, errorCode? }` | Mac + web caller | **native** (2026-09-15). `ASAuthorizationAppleID`. Cancel → `{ cancelled: true }` (no banner). Fail → human `error` (unverified Apple email / 1000 is not the raw `AuthorizationError`). Web also maps that string in `App.jsx`. **Native sheet is IPA-only.** The iPad auth column always shows the Apple circle. Phone / desktop still hide it unless `isEdgeiOSShell`. A tap outside the shell does not call `signInWithOAuth({ provider: 'apple' })`. |
...
| `setNativeCallChrome` | JS→native | `{ minimized?: boolean, videoVisible?: boolean, controlsHidden?: boolean, participantAvatars?: [{ identity, name?, avatarUrl? }] }` | `{ ok: boolean }` | Mac | **native** (2026-08-27). Hide the native video overlay when the web chrome is minimized. **2026-09-04:** `participantAvatars` names/photos for IPA video tiles (camera-off people). **2026-09-04:** `controlsHidden` slides the 1:1 You-inset with the pill. |
...
| `dismissKeyboard` | JS→native | none | `{ ok: boolean }` | Mac | **native** (2026-08-27). Answer / start call drops the WKWebView keyboard. CallKit answer also resigns first responder without waiting for JS. **2026-09-11:** web helper `dismissEdgeKeyboard` in `edgeNative.js`. Account info matches Edit profile scroller so IPA swipe-down (`keyboardDismissMode = .interactive`) works. |
| `setKeyboardAccessory` | JS→native | `{ visible: boolean }` | `{ ok, visible }` | Mac + web caller | **native** + **web caller** (2026-09-12). Native default is the WK Done / prev-next bar **on**. `EdgeKeyboardAccessorySync` only opts **out** for GIF search. **2026-09-16:** default-on avoids mid-rise `reloadInputViews` (iPhone sim clipped keys or late bar pop). New IPA required. |
| `setOrientationLock` | JS→native | `{ lock: 'portrait'\|'none' }` | `{ ok, locked?, skipped?: 'ipad', unchanged? }` | Mac + web caller | **native** + **web caller** (2026-09-21). iPhone IPA only. Composer open → `portrait` (forces rotate). Last composer close → `none` so a still-sideways phone can go landscape again. iPad / old IPA / Safari no-op. New IPA required. **Safari / PWA / Android:** `ComposerPortraitRotateHint` (web **`1.4.466`**) shows a rotate-to-portrait icon in landscape until the phone is upright. **`1.4.467`:** landscape composer taps do not raise the software keyboard (it sat over the hint). |
| `getStorefront` | JS→native | none | `{ countryCode: string, isUnitedStates: boolean }` | Mac | **native** (StoreKit 2 `Storefront.current`, 2026-09-05). ISO 3166-1 alpha-3 (`USA`). IPA prints a cheaper web dollar only when `isUnitedStates`. Missing method / unknown → treat as not US. **Device smoke pending.** |
| `getStoreProducts` | JS→native | `{ productIds: string[] }` | `{ products: Array<{ id, displayName, displayPrice, productSlug, introEligible?, introDisplayPrice?, introPeriodCount?, introPeriodUnit?, introPaymentMode?, introSource? }> }` | Mac | **native** (StoreKit 2, SK1 intro fallback). `introSource` is `storekit2` / `storekit1`. JS founding fallback if both omit the offer (`$16.99` Starter monthly). Card + iPhone CTA use that price. New IPA for SK1. **`1.4.73`.** |
| `purchaseStoreProduct` | JS→native | `{ productId, appAccountToken? }` | `{ ok, status, transactionId?, originalTransactionId?, signedTransactionInfo? (JWS), expiresAt? }` | Mac | **native** + web SubscribeModal / fan SUB / Edge Pro. Confirm via Edge `apple-iap-verify` (deployed **test + prod** 2026-09-05). Restore grants an already-paid sandbox sub. |
...
| `scanDocument` | JS→native | `{ purpose?: string, maxPages?: number }` | `{ ok, cancelled?, unsupported?, images?: [{ mimeType, base64, width, height }] }` | Mac + web caller | **native** + **web caller** (2026-09-11, review queue **2026-09-12**, Offers **2026-09-12**). VisionKit `VNDocumentCameraViewController`. Default 1 page, max 8. JPEG ≤2000px. W-2G **Take photo** asks for up to 8 pages. Offers mailer **Take photo** uses the same camera. Every W-2G page opens extract/review (save / download / adjust / share / remove). Multiple pages swipe in the image frame. Save or Remove keeps the screen and slides in the next remaining slip. |
...
| `pickPhotos` | JS→native | `{ purpose?: string, maxCount?: number }` | `{ ok, cancelled?, images?: [{ mimeType, base64, width, height }] }` | Mac + web caller | **native** + **web caller** (2026-09-11, app-wide **2026-09-12**). `PHPickerViewController`, images only. Default 12, max 40. JPEG ≤1600px. Wired on W-2G bulk, Lounge compose (feed / thread / reply / quote), chat images, profile banner/avatar, chat group photo, Offers mailer library. Cancel / old IPA / PWA → `<input type="file">`. Video uses `pickEdgeVideo` on a current IPA. |
| `pickEdgeVideo` | JS→native | `{ purpose?: string, maxSourceBytes?: number, maxUploadBytes?: number, maxClipSeconds?: number }` | `{ ok, cancelled?, assetId?, previewUrl?, duration?, width?, height?, byteSize?, posterJpegBase64? }` | Mac + web caller | **native** + **web caller** (2026-09-22). `PHPicker` videos, one clip, copied into caches. `maxSourceBytes` caps the copy (Lounge and chat send 8 GB). Chat `maxUploadBytes` is the Lounge cap clamped to a 5 GB R2 PUT. Omitted fields keep the old 1.5 GB / 200 MB / 60s behavior. `previewUrl` is `edge-video://local/<id>` and is only for the trim player. The composer shows the JPEG poster. An image request for that URL is rejected so a long clip cannot be decoded as a picture. Cancel / old IPA / PWA → `<input type="file">`. Web crop/trim modal stays the UI. |
| `exportEdgeVideo` | JS→native | `{ assetId, startSec, endSec, cropPx?, intrinsicWidth?, intrinsicHeight?, maxClipSeconds?, maxUploadBytes? }` | `{ ok, passthrough?, assetId, sourceAssetId, byteSize, previewUrl }` | Mac + web caller | **native** (2026-09-22). Re-encodes with `AVAssetExportPreset1920x1080` only for a trim, a crop, a `.mov` / spatial / screen recording, or a file over the caller's upload cap. Other clips return `passthrough: true`. Caps clamp to 20:00.35 and 8 GB. Omitted fields stay 60.35s / 200 MB. `cropPx` is the modal rect in displayed pixels (y down). Progress: `edge-native-video-progress` `{ phase, progress, assetId }`. Phases are `checking`, then `encoding` only when the clip is re-encoded, then `upload`. A passthrough clip skips `encoding`. Older IPAs send `export` for that encode step. **iOS 26+:** the encode and the upload register a continued-processing task so locking the phone does not suspend the job. Lock screen shows Preparing video while any clip is encoding, then Uploading video. One export runs at a time, so the next clip can encode while an earlier file is still uploading. New IPA required. |
| `uploadEdgeVideoTus` | JS→native | `{ assetId, accessToken, supabaseUrl, anonKey, streamMaxDurationSeconds?, maxUploadBytes? }` | `{ ok, streamVideoUid, assetId }` | Mac + web caller | **native** (2026-09-22). Lounge only. Tus create (`lounge-cf-stream-tus-create`) then PATCH to Cloudflare. Files over 200 MB are split into 100 MB chunks (Cloudflare's single-request cap). The Edge function overwrites `maxDurationSeconds` from the Edge Pro grant. Progress phase `upload`. |
| `uploadEdgeVideoPut` | JS→native | `{ assetId, uploadURL, contentType?, cacheControl? }` | `{ ok, assetId }` | Mac + web caller | **native** (2026-09-22). Chat only. JS mints the R2 presign, native PUTs the MP4. |
| `cancelEdgeVideo` | JS→native | `{ assetId? }` | `{ ok: true }` | Mac | **native** (2026-09-22). With `assetId`, cancels that clip only. With no id, cancels every in-flight export and upload. |
| `share` | JS→native | `{ url?: string, text?: string, title?: string, images?: [{ mimeType, base64, filename? }] }` | `{ ok, cancelled? }` | Mac + web caller | **native** + **web caller** (2026-09-12). `UIActivityViewController`. URL and/or text and/or up to 4 JPEG/PNG images. Wired on Lounge post / profile, W-2G scan image, poker session result, market chart snapshot, chat recording link. Old IPA / PWA keep `navigator.share` then clipboard. New IPA required. |
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds?, pausedAt?, pauseElapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, overlay?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit for **background / Lock Screen** only. **2026-09-17:** in-app fake Island retired (could not nudge system status chrome). Title-bar live chips own in-app again. Tap → manager tab only. New IPA required. |
| `setLiveBankrollIslandOverlay` | JS→native | `{ visible: boolean }` | `{ ok, visible }` | Mac + web caller | **retired (2026-09-17).** No-op on current IPA. Kept so old web builds do not throw. |
...
| Universal Links (email confirm) | Both | **Mac 2026-09-11:** Associated Domains `applinks:edgetilt.com` + `lvslotpro.com` (+ www). `AppDelegate` / SwiftUI `NSUserActivityTypeBrowsingWeb` → `EdgePushManager.handleUniversalLink` → same WKWebView pending queue as APNs. Path **`/auth/confirm` only**. No new `EdgeNative` method. **Web:** `/auth/confirm?token_hash=&type=` + AASA. Templates in **`docs/supabase-auth-email-templates.md`** (Ryan pastes dashboard). New IPA required for UL + `edgetilt://auth/confirm` (maps to the https confirm URL). **2026-09-11:** cold-start UL is read from `launchOptions` (not only `continue userActivity`) so the WKWebView actually loads `/auth/confirm?token_hash=`. Same-webview re-attach no longer parks the URL in pending forever. Gmail in-app ignores UL; browser stays on a viewport-pinned confirm card and waits for **Confirm** before `verifyOtp` (Gmail copy/preview otherwise burns the hash), then **Open EdgeTilt**. IPA still auto-verifies because a Universal Link open is a real tap. |
...
| WKWebView `alert` / `confirm` / `prompt` | Mac | **Native (2026-09-12)** + **Ryan sign-off 2026-09-12:** custom `uiDelegate` implements the JS dialog panels. Play Logbook delete, poker deletes, chat leave-group show stock Cancel / OK. W-2G delete also uses AppShell in-app confirm. In-app sheets still optional where they look better. |
| W-2G extract keyboard accessory | Mac + web | **2026-09-12:** ripped the custom hover chip. Tax-field focus calls `setKeyboardAccessory({ visible: true })` for the stock WK Done / prev-next bar. Old IPA: no bar (same as before the custom chip). **Same night:** AppShell `EdgeKeyboardAccessorySync` does this for every real field except GIF search. |
...
- **Platform Slots Edge + Edge Pro:** IAP primary when StoreKit returns the product; **Subscribe on the web** is secondary. **Storefront gate:** cheaper web dollars (founding chip, `$9.99` on Edge Pro web CTA, fan web sticker) print only when `getStorefront.isUnitedStates`. Both buttons stay. IPA Subscribe carousel is Starter + Pro + Lifetime (Apple cleared the `$1,000+` price-point request **2026-09-14**). Restore + Terms/Privacy on the paywall. Apple-billed manage → `manageStoreSubscriptions`.
...
- **Still owed before real charges:** sandbox smoke, ASC Server Notification URL paste, full x5c chain to Apple Root, counsel + Review notes. Storefront gate landed 2026-09-05 (`getStorefront`). Refund sheet + ASSN V2 (`apple-iap-notify`) landed 2026-09-05. May upcharge IAP for Apple’s cut.
...
**Payloads:** VoIP JSON and the fallback APNs `userInfo` carry `avatarUrl` from `profiles.avatar_url`. Foreground Realtime already passes it on `reportIncomingCall`. A **deduped** later report prefetches only. `https` only. First call from a new person may be empty if the URL is not cached yet; the repeat should hit disk. Test `lounge-send-activity-push` must be redeployed for home-screen VoIP to carry the URL. Prod Edge not touched.
...
**Fix:** add `audio` to `UIBackgroundModes`; `beginCallBackgroundTask` from report/answer until end; skip SW hygiene when a CallKit call is already tracked; Simulator never waits on SW hygiene (iOS 27 `removeData` can hang); device hygiene has a 2s cap; CallKit answer is `preferAccept: true`, `openRoom: true`, full call modal.
...
| Video | Native | Overlay behind WKWebView. **One tile per LiveKit participant** (camera or avatar). 1:1 stays main + PiP; 3+ is a grid. Web stage is a transparent hole. |
