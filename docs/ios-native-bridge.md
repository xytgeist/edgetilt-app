...
| `setNativeCallStreamFocus` | JS→native | `{ isLocalMain?: boolean, focusedIdentity?: string, quadFocus?: boolean }` | `{ ok: boolean, isLocalMain: boolean, focusedIdentity?: string, quadFocus?: boolean }` | Mac | **native** (2026-08-29, **2026-09-04**). Count-based tiles: 2 = featured full-bleed + inset (tap inset to swap; `isLocalMain` / local `focusedIdentity` features You); **3–4 = featured full-bleed + right stack**; 5+ = featured top + 2-row grid. `quadFocus` is ignored. |
