...
| `setNativeCallChrome` | JS→native | `{ minimized?: boolean, videoVisible?: boolean, participantAvatars?: [{ identity, name?, avatarUrl? }] }` | `{ ok: boolean }` | Mac | **native** (2026-08-27). Hide the native video overlay when the web chrome is minimized. **2026-09-04:** `participantAvatars` names/photos for IPA video tiles (camera-off people). |
| `setNativeCallStreamFocus` | JS→native | `{ isLocalMain?: boolean, focusedIdentity?: string }` | `{ ok: boolean, isLocalMain: boolean, focusedIdentity?: string }` | Mac | **native** (2026-08-29, **2026-09-04**). Full-screen is the focused **remote**. You stay lower-right. `focusedIdentity` is a remote LiveKit id (empty = first remote). `isLocalMain` is ignored (kept so old JS does not throw). |
