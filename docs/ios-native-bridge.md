...
| `setKeyboardAccessory` | JS→native | `{ visible: boolean }` | `{ ok, visible }` | Mac + web caller | **native** + **web caller** (2026-09-12). Default IPA hides the WK Done / prev-next bar. `EdgeKeyboardAccessorySync` in AppShell turns it back on for focused fields (Lounge / thread / chat / logbook / bankroll / poker / W-2G / settings). GIF search stays hidden. **2026-09-16:** showing the bar defers `reloadInputViews` until `keyboardDidShow` (plus a short fallback) so iPhone Simulator does not clip the keyboard to accessory height. New IPA required. |
...
| `syncLiveBankrollActivity` | JS→native | `{ slots?: { id, label, startAt?, elapsedSeconds? } \| null, poker?: { id, label, paused?, startAt?, elapsedSeconds? } \| null }` | `{ ok, supported?, disabled?, started?, updated?, ended?, error? }` | Mac + web caller | **native** + **web caller** (2026-09-16). ActivityKit Live Activity for live slots / poker sessions. Compact Island + Lock Screen. Tap → `edgetilt://live-session?tab=bankroll` / `poker-bankroll`. Web chip hides on IPA when this reports `ok` and not `disabled`. Old IPA / PWA keep the title-bar chip. New IPA required. CallKit still owns the Island during a Chat call. |
