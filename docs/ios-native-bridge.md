...
| `setLiveBankrollIslandOverlay` | JS→native | `{ visible: boolean }` | `{ ok, visible }` | Mac + web caller | **native** + **web caller** (2026-09-17). Show / hide the in-app fake Island. Web sets `visible: false` on bankroll screens that already show the live session card. Overlay also auto-hides when the app resigns active (system Island takes over). |
