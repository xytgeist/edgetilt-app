...
| `exportEdgeVideo` | JS→native | `{ assetId, startSec, endSec, cropPx?, intrinsicWidth?, intrinsicHeight? }` | `{ ok, assetId, sourceAssetId, byteSize, previewUrl }` | Mac + web caller | **native** (2026-09-22). `AVAssetExportSession` `AVAssetExportPreset1920x1080`, H.264+AAC, 60s cap, 200MB cap. `cropPx` is the modal rect in displayed pixels (y down). No crop when the rect is the full frame. Progress: `edge-native-video-progress` `{ phase: 'export', progress, assetId }`. |
