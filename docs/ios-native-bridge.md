...
The Dynamic Island / top incoming circle reads an undocumented setter, **`localizedCallerImageURL`**. Helper lives in **`EdgeCallKitManager.swift`** (do not split it into its own target file ... Xcode dropped that once and the IPA had no avatar code). We set it via `responds(to:)` + `perform` on the `CXCallUpdate` **before** `reportNewIncomingCall`, using a tmp JPEG if we already have bytes, otherwise the `https` URL. App Review can theoretically flag that selector name. If Apple strips it, the circle goes empty again and we do not have a public replacement.
