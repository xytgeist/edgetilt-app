...
| Deep link handling after APNs | Both | Native opens absolute `url` from the APNs payload (`userInfo.url`). Web already has `?tab=` / lounge parsers. **Mac remaining:** `UNUserNotificationCenter` tap → load that URL in the WKWebView. Windows payload shipped 2026-08-25. |
...
| Skip web push SW in shell | Windows | **Done (2026-08-24):** `useWebPushNotifications` unsupported under `isEdgeiOSShell()`; no `push-sw` register. A2HS / install-for-push gated via `iosPwaInstallRequired`. **Lounge Settings native APNs toggle** wired 2026-08-25. **Token upload** to `apns_device_tokens` wired 2026-08-25. Send is Edge `APNS_*` + redeploy `lounge-send-activity-push` / `send-test-push` / `send-due-offer-reminders`. |
