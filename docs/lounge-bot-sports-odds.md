...
**Settlement (`grade_picks`):** Odds API `/scores?daysFrom=3` first (kickoff + 90m **and** `completed`). If that row is missing, **TheRundown** `STATUS_FINAL` (Edge can actually call it), then ESPN public scoreboards (NFL, CFB, NBA, NCAAB, MLB, NHL, UFC). ESPN often **403s** from Supabase Edge. MMA Odds API `/scores` drops finished fights. UFC moneyline uses 1-0 from the winner. Still never grades live events. Metadata `grade_source` is `odds_api`, `rundown`, or `espn`.
