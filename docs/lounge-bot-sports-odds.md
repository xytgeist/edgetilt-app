...
| Lounge | Public Signal feed as today | Public **teaser** + **`creator_fan_only`** full card (desk-list thread only when the slate has **2+ games**) |
...
**Primetime Preview vs Publish:** Preview and Publish share `findPrimetimeGameCandidate`. Desk lines are the **house slate votes** for that one game (`buildNflAtsSlateCard` + pasted `syndicate_betting_splits`). Printed ATS numbers shop the best remaining us/us2 book after the vote (most points, then juice). Costume EPA-sign / fake-RLM Chedda is gone. Chedda **PASS** unless dog+hook or pasted sharp money. Dog+PVAL is Scott … not a Chedda clone. Same-side public+money (e.g. 73/75 vs 27/25) is not RLM and is not a Chedda unlock. Ticket/handle opposition is **fade the public** (`isFadePublic`) … the ⚡ tape prints `Fade public · …`. **RLM** is only when `lounge_market_files` open→current spread walked ≥0.5 pts against the ticket-heavy side (`applyReverseLineMovement`) … ⚡ then prints `RLM · open→current against … tickets`. Chedda’s desk why matches: **`RLM significant.`** / **`Fade the public.`** / **`Sharp money divergence.`**. Rocco short-fav-only still uses `countsForHouse: false` internally, but the card why is only **`Short-fav on {team}.`** … never “does not count as a house vote.” 1-1 house split is **⚔️ House Divided** (Scott DEN / Rocco KC), not a crowned lean, and does not ledger a fake majority. PASS desks print the vote only. Tank has two votes: ATS in the desk list, O/U under the tape (**O/U** header + purple Tank model line). “wind kills the bet” is italic. Splits line is the ticket/money board only … no `Pasted (action_pro)` slug. Synthetic hash boards never print on the card. All-pass ATS → Lean is **PASS**, no spotlight ledger row. Publish used to append `nfl` + `primetime` onto `community_feed_posts.category_pills`, which the Lounge allowlist rejects (`sports` only). Insert failed, Ops toasted “no game.” Fix: sanitize pills in `publishLoungeBotPost` and keep primetime on `sports`. Publish now matches Preview: **public Lounge + VIP chat + X** all get the same 4-desk card. No fan-only Lounge post. Footer: lean not lock … desks stay as written; kill/flip only on a listed starter out or if the number walks off the edge; official lock is 90-min inactives. MNF lean cron is **2:00pm PT** (TNF / SNF stay 3:30pm). Friday house lean is not the primetime lock ... spotlight voids leftover pending desk rows on that event. **Second drop** `nfl_primetime_lock` (~90 min pre): confirm the lean or kill (pass, cancel the ledger row). Hold if QB / LT / edge is still Q / GTD / doubtful. No auto-flip. Cron polls every 10 min in the Thu/Sun/Mon window; Edge only publishes inside 70-110 min pre-kick. Ops **Primetime lock** Preview/Publish can run anytime. **`lounge-odds-poll` is on prod** (`jtjgtucumuoswnbauxry`) as of 2026-09-10. Primetime cards still print the weather/stadium line. Location comes from Rundown `venue_location` / `venue_name`, not the listed home team's regular house. Melbourne / London / Mexico City / Munich / Frankfurt / Sao Paulo are seeded so Tank weather hits the actual site. If Rundown names a city we do not have coords for, weather stays off rather than stamping SoFi because the Rams are listed home.

**Ops Scorecard (`sharpesyndicate.com/ops`):** `lounge_bot_get_picks_record(bot, timeframe, sport)`. Prod has the 3-arg from **`20260828113000`** (applied 2026-09-11). The old 1-arg left tiles at 0-0.

**Public site ledger (`sharpesyndicate.com` Overview / Audited Ledger):** Overview tiles flip All / NFL / CFB / UFC every 4s (dots jump). CFB book was wiped 2026-09-14 (week 1-3 no longer count) ... next Friday slate starts clean. Ledger filters and row pills label **4-0 Hammers** and **2-1+ Consensus**. UFC Tank round O/U is parked until trained ... he still cards an ML and votes with the house. **That live UFC engine is costume.** Locked rebuild contract: **`docs/syndicate-ufc-desk-contract.md`**. Do not code the new desks until the independence test in that file is a real check. Football Tank totals unchanged. Stored `consensus_badge` wins when present. `fetchSyndicateLedger` loads **Syndicate** `won` / `lost` / `push` only (resolve `@sharpesyndicate` then paginate). Do **not** take the newest-kickoff slice … Saturday pending fills that cap and finished grades fall off. Pending/future games cannot be in the W-L tiles. **CLV Beat Rate** = beats / (beats + misses) from locked `clv_pts` … even CLV is off the board. Client pins **production** Supabase (`VITE_SYNDICATE_SUPABASE_*` override only … not EdgeTilt test `.env`). Hosted on **Cloudflare Pages** project **`sharpe-syndicate`** (Operations account). Git-connected to **`xytgeist/edgetilt-app`**. **Production branch is `test`** … `git push origin test` publishes **sharpesyndicate.com**. `main` is Preview only. Direct Upload fallback: **`npm run syndicate:deploy`**.

**Settlement (`grade_picks`):** Odds API `/scores?daysFrom=3` first (kickoff + 90m **and** `completed`). If that row is missing, **TheRundown** `STATUS_FINAL` (Edge can call it), then ESPN public scoreboards (NFL, CFB, NBA, NCAAB, MLB, NHL, UFC). ESPN **403s from Supabase Edge** even with a Chrome UA. MMA Odds API `/scores` drops finished fights. TheRundown MMA (`sport_id` 7) can also return **0 events** the same night (Paris 2026-09-05). UFC moneyline uses 1-0 from the winner. Still never grades live events. Metadata `grade_source` is `odds_api`, `rundown`, `espn`, or `espn_local` (Windows one-shot when Edge cannot see a result source).
...
**Moneyline print gate (every sport, including UFC Scott and these edge alerts):** sit unless the win-rate gap clears `2 + 0.40 / p_mkt` (`mlWinGapClears` in `loungeBotOddsCaption.ts`). That is win-rate points, not ROI. The EV percents in the table still gate **spreads and totals**. Book floors stay and stack with the ladder. A miss is a sit, not a smaller bet. The printed ML line is the fair price and the point gap, not `+% EV`.
...
**Desk cards pin to that file for the vote, then shop the printed ATS number.** After the side is picked, `bestSpreadForTeam` takes the best us/us2 point (then juice) for that side. A 1-1 can print Scott Broncos +2.5 / Rocco Chiefs -1.5 off different books. After lock / kickoff, `buildNflAtsSlateCard` uses locked `close_spread_home` / `close_total` (`resolvePregameSpreadFromFile` / `resolvePregameTotalFromFile`) and does **not** shop live. It never takes the first Odds API book after kickoff ... that is often a live in-play DraftKings blowout (Week 1 Pats/Hawks posted Seattle +7.5 off live DK while the locked close was Seattle **-3**). Pregame without a file still prefers sharp quote, then first book, then shops the print. Missing close after kickoff → skip the game (do not invent). Totals stay on the file / consensus.

**PVAL injury residual ledger (v2 scaffold):** table **`nfl_pval_injury_events`** (migration **`20260903220000`**). On NFL polls, first hard-OUT inserts booked PVAL + spread snapshot; when close locks, fills `residual_home = spread_move − expected` (away OUT expects more-negative home spread). Manual review only ... never auto-rewrites `nfl_player_pvals`. Writer: **`loungeBotPvalInjuryLedger.ts`**. **Force:** `node scripts/force-pval-ledger-poll.mjs --target=test|production|both` → Edge action **`pval_injury_ledger`** (no Lounge posts; works if bot is stopped).

**Monthly scoreboard (ops):** Edge action **`syndicate_monthly_scoreboard`** (`loungeBotSyndicateScoreboard.ts`) … ATS + CLV by desk × bucket (hammer / consensus / divided / pass). Scott/Rocco/Chedda = sides; Tank = totals only. Every row includes sample **n**. CLV = **your side vs locked close** (not opener). **CLV beat %** = beats / (beats + misses) … calling the same number as the close is off the board (same idea as an ATS push). Trust floor **n ≥ 25** per bucket×desk before crowning. Desk rollup mixes buckets ... informal only, never "shop ATS." **Primary UI:** **https://sharpesyndicate.com/ops** (admin login) → Sharp Desk → **Monthly Board**. Desk panel is **not** on EdgeTilt Signal (`sports-odds` / `/?tab=bots`). **Do not trust adaptive persona weights** until a bucket has a real sample. FEI still waits.
...
| --- | --- | --- |
| **`in_game_edge`** | Live game (commenced, not completed per scores API) | Spreads/totals **+EV ≥ `min_live_edge_pct`** (default **7.5%**); ML uses `2 + 0.40 / p_mkt`; **≥ 6 books**; **no live soccer draw ML**; block live ML **> +800** unless **≥ 8 books**; footer **Live · verify quickly** on flagged longshots |
...
**Live pick guards (`loungeBotLivePickGuards.ts`):** pre-match **edge** uses **`loungeBotEdgeAlertThresholds.ts`** (see table above); Coffee & context still use **`DEFAULT_MIN_BOOKS = 3`**. Live spreads and totals still need **`min_live_edge_pct`** (default **7.5%**). Live moneylines use the same `2 + 0.40 / p_mkt` gap as pre-match. Min **6** books, suppress **soccer draw ML** in-play, block live ML longer than **+800** unless **≥ 8** books, optional **Live · verify quickly** / **Live · extreme number** footer.
...
4. Spreads and totals still need **`min_best_bet_hour_ev_pct`** (default **6%**). Moneylines use the `2 + 0.40 / p_mkt` gap instead. **≥ 5 books**; stale cap **15%**
...
Desk mapping: **Scott** = PASS unless model−market ≥ 2.5 after PVAL (1.5 only on true 3/7 keys; NFL model = EPA + ESPN trench) · **Rocco** = PASS unless short-fav / hurtSide / hook-tax / pasted chalk-trap / NFL trench mismatch ≥0.8 pts (short-fav alone is a desk lean only … `countsForHouse: false` … hammer / consensus / house divided treat him as PASS; juice worse than **-115** → always PASS / hard ugly-juice gate) · **Chedda** = PASS unless dog+hook / pasted sharp money (dog+PVAL is Scott; no dog+raw-EPA; no synthetic) · **Tank** = totals spine (model ≥3.5 or ≥2.5 key-cross 48/51/54) + **wind veto on Overs** (outdoor ≥15 mph) + **falling-total veto on Overs** (`open_total` from `lounge_market_files`) + **CFB non-conference Over bump** to 3.0 when wind is clear and total has risen (or hard edge ≥3.5). Shown on slate root only … no VIP desk thread part. **Tank spots (sidecar ATS):** Always printed next to his O/U. PASS unless two independent Tank reasons agree (`rest` / `weather` / CFB `tempo` / `total_agree`), **or** his Under + dog tell fires (that one is enough). Injury is a stack tag only. Over + weather-dog = PASS the side. Under + dog = the side. Pasted Action/VSiN boards are a weighted street score (never a 6th reason, never fire alone; strong fade vetoes the spot). Tank's published ATS spot counts in the house tally. Hammer = **4-0 only** (all four desks same side). A PASS kills the hammer and it becomes consensus. Consensus = 3-1 / 3-0 / 2-0. House Divided = everything else. Synthetic splits never score. Primetime = spotlight lean, not house card.

**PVAL injury prior (v0/v1):** table starts Sleeper-owned. `is_custom_override` is off unless Ops Save or the per-row switch locks it. Unmatched Rundown OUTs use position-band **Typical** defaults (`loungeBotPvalBands.ts`). **v1 fill / weekly refresh:** `npm run syndicate:pval-sleeper` (Sleeper depth → band; **multi-week** `pts_ppr` avg lookback 4 with current-week 55% weight + soft `search_rank` blend → seat); `--apply` inserts new; `--refresh` upserts non-override rows. Does **not** auto-protect seed notes (`--protect` is opt-in). Tuesday GHA cron runs `syndicate:pval-sleeper:refresh:{test,production}` with the football metrics workflow. Each apply writes `nfl_pval_sync_runs` (pulled / wrote / movers). Ops **PVALs** tab (`PvalSyncDumpCard`) shows the latest Sleeper dump so you can see data in and rows written. Heartbeats stay pass/fail and do not overwrite the dump. **OL / trench:** Sleeper OL has almost no `depth_order` … v1 keeps active OT/IOL via roster + `years_exp` seat (top 3 OT / 4 IOL per team). **TWO·DEEP hole-fill** (`npm run syndicate:pval-twodeep`) scrapes `thetwodeep.com` NFL team pages after Sleeper (Hunter Ansley permission; sequential, 4s between live pages, 20h cache). It only patches OL years_exp seats, inserts missing OL starters Sleeper never seated, and upgrades TE2/WR2-3 when Two Deep has a clear starter and there is no fantasy seat. Curated overrides and fantasy-seated PVALs stay. Does not replace nflverse EPA, ESPN trench, or Rundown injury status. Tuesday GHA runs apply on test + prod after the Sleeper refresh (`continue-on-error`). **TE:** depth 1 → `te1`; depth ≥2 or missing → `te2` (0.1–0.4, typ 0.2). **QB replacement delta:** starter OUT → `max(0, starter − healthyBackup)` (typical backup 0.25 if unknown; 0 if every mapped backup also out); backup-only outs → 0. Starter identity prefers `band_key` / depth-1 / highest-PVAL OUT QB vs healthy roster (not a raw `pval > 0.5` cutoff). **Status priors:** hard OUT 1.0 · doubtful 0.8 · GTD 0.5 · questionable 0.4. **Calibration ledger:** `nfl_pval_injury_events` logs first hard-OUT vs close residual (no auto table rewrite; do not retune bands until **n ≥ 40** clean hard-OUT residuals). Non-QB stack soft/hard caps 1.2 / 2.0; when QB impact ≥ 2.0, ×0.55 on **offensive** non-QB only (defense unchanged). Scott still clears **2.5** after the adjust. K/P/coach = 0.

**ATS slate auto-publish (pg_cron):** CFB Fri **12pm PT** house (`cfb_slate_card`); Wed **2pm** VIP midweek Thu/Fri nights (`cfb_wed_midweek_vip`); Thu **3:30pm** CFB night tease (`cfb_thu_night_spotlight` … all-pass is **All pass**, not a fake home-number Lean); Sat **10am** CFB adds/kills (`cfb_sat_vip_adds_kills` … VIP only, silent unless a number walked **≥1.5 against the lock** or the locked side **crossed favorite/dog**; a still-dog away lock is not a kill). Sat **9am PT** UFC slate (`ufc_slate_card` … public + fan-only + VIP are **Scott + Rocco**; **next fight-night cluster only**, not the full MMA dump / Dec 31 futures / a same-day Euro card glued on; Tank round O/U parked until trained). Chedda and Tank are not printed. Fights are grouped **🎯 Consensus** (same fighter), **⚔️ House Divided** (different fighters), **📐 Lean** (one desk voted, the other sat), and **⏭️ Pass** (both sat). Public Lounge shows one fight (names and prices, no sit math) from each section with more than one fight. A section with a single fight stays a count. Pass is never sampled. Fan-only and VIP chat have the card. No gold on the matchup or the weight class. Scott sits a moneyline unless the win-rate gap clears `2 + 0.40 / p_mkt`. A price longer than +1000 plus a thin last-5 also sits him. Rocco independence check: `npm run syndicate:ufc-rocco-independence`. Contract: [`docs/syndicate-ufc-desk-contract.md`](./syndicate-ufc-desk-contract.md). NFL Fri **1pm** is a **lean** (`nfl_slate_card`), not a lock. Sat **7pm PT** steam confirm/kill (`nfl_sat_steam`). Sun **~8:30am PT** early-window inactives lock (`nfl_sunday_early_lock`, 1pm ET cluster). Sun **~11:30am PT** late-window lock (`nfl_sunday_late_lock`, 4:05/4:25 ET). SNF/TNF/MNF stay on primetime lean + 90-min lock. Migration `20260911190000`. Same Sat flip helper is on `nfl_sat_vip_adds_kills` (Ops leftover).
...
**Cron:** [`.github/workflows/syndicate-football-metrics-sync.yml`](../.github/workflows/syndicate-football-metrics-sync.yml) … Tuesdays **14:00 UTC** and catch-up **18:00 UTC** (GitHub often delays or skips the first slot) sync **test + production** (NFL EPA + CFB power + UFC metrics + **Sleeper PVAL refresh** + **TWO·DEEP hole-fill**). Manual dispatch can set `sync_production=false` to skip prod. Edge Monitor heartbeat **`syndicate_football_metrics_sync_production`** plus per-step `syndicate_weekly_*` ids. Ops **Weekly Pulls** tab shows pass / waiting / fail (fail after noon PT if nothing wrote). Staff SELECT **`20260910230000`**. The schedule file only fires from **`main`**.

**UFC metrics roster:** Table started as a hand-seeded ~38 champ/contender list (`20260830230000`). Tuesday `syndicate:sync-ufc-metrics` refreshes those rows from **ufcstats.com** and **inserts anyone on upcoming + the last 3 completed UFC cards** who is not on the roster yet. `--ensure-only --ensure="Name"` adds named fighters without rescraping the whole roster. Name match for Scott and Rocco is the full name, an alias, or one unique last name (`O'Malley`). A shared first name does not attach someone else's file. Names UFC Stats does not list get a Sherdog **result tape** (`syndicate:sync-ufc-sherdog`, `counts_measured` false): win / method / round only. Rocco scores finish style and form on that tape and does not read blank takedown or strike counts as zero. Scott ignores those rows (`career_measured` false) so placeholder career rates never become a fair price. The fight label still uses the stored weight class, not the word UFC. Same job writes **`ufc_metrics_sync_runs`** (what changed: new names + career moves past a noise floor). Ops **UFC Metrics** tab shows the latest dump, same idea as the Sleeper PVAL dump. Same job upserts **`ufc_card_fights`** (venue, Apex, scheduled 3/5) and **`ufc_fighter_aliases`**. `--cards-only` skips the career refresh and does not write a dump. This is not a full UFC encyclopedia… quiet names stay off until they hit a card, we ensure them, or Sherdog fills a result tape.
...
**Trench follow-up scheduler (do not skip):** spec [`data/syndicate/trench-followup.json`](../data/syndicate/trench-followup.json). Ticket [issue #3](https://github.com/xytgeist/edgetilt-app/issues/3). Order after z-score: recalibrate 0.56 on a 2026 board → scheme mixer (air-yards share / 12-personnel, mix caps) → sack/hit process≠outcome haircut. First nag **Tue 2026-09-29**. Test pg_cron **`syndicate_trench_followup_nudge`** (`20260910160000`, **test only**) posts [lvslotpro.com/theo](https://lvslotpro.com/theo) weekly until `syndicate_trench_followup.status='done'`. GHA [`.github/workflows/syndicate-trench-followup.yml`](../.github/workflows/syndicate-trench-followup.yml) is the same nag on `main` (GitHub schedule only runs from the default branch). Kill switch: JSON `status=done` + test SQL `update syndicate_trench_followup set status='done' where id='nfl'`.
...
**Cron / scheduled auto-publish:** today's destinations. A public Lounge post always tweets. X gets the public tease, except NFL primetime (TNF / SNF / MNF), which tweets the same 4-desk card as VIP chat. Wed TNF VIP is fan-only Lounge + VIP chat (no public, no X). Other VIP-only drops stay chat-only.
