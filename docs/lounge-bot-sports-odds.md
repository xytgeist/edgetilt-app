...
**Desk cards pin to that file, not live first-book.** After lock / kickoff, `buildNflAtsSlateCard` uses locked `close_spread_home` / `close_total` (`resolvePregameSpreadFromFile` / `resolvePregameTotalFromFile`). It never takes the first Odds API book after kickoff ... that is often a live in-play DraftKings blowout (Week 1 Pats/Hawks posted Seattle +7.5 off live DK while the locked close was Seattle **-3**). Pregame without a file still prefers sharp quote, then first book. Missing close after kickoff → skip the game (do not invent).
