...
**Primetime Preview vs Publish:** Preview and Publish share `findPrimetimeGameCandidate`. Publish used to append `nfl` + `primetime` onto `community_feed_posts.category_pills`, which the Lounge allowlist rejects (`sports` only). Insert failed, Ops toasted “no game.” Fix: sanitize pills in `publishLoungeBotPost` and keep primetime on `sports`. Publish now matches Preview: **public Lounge + VIP chat + X** all get the same 4-desk card. No fan-only Lounge post. Footer: lean not lock … desks stay as written; kill/flip only on a listed starter out or if the number walks off the edge; official lock is 90-min inactives. MNF lean cron is **2:00pm PT** (TNF / SNF stay 3:30pm). Friday house lean is not the primetime lock ... spotlight voids leftover pending desk rows on that event. **Second drop** `nfl_primetime_lock` (~90 min pre): confirm the lean or kill (pass, cancel the ledger row). Hold if QB / LT / edge is still Q / GTD / doubtful. No auto-flip. Cron polls every 10 min in the Thu/Sun/Mon window; Edge only publishes inside 70-110 min pre-kick. Ops **Primetime lock** Preview/Publish can run anytime. **`lounge-odds-poll` is on prod** (`jtjgtucumuoswnbauxry`) as of 2026-09-10. Primetime cards still print the weather/stadium line. Location comes from Rundown `venue_location` / `venue_name`, not the listed home team's regular house. Melbourne / London / Mexico City / Munich / Frankfurt / Sao Paulo are seeded so Tank weather hits the actual site. If Rundown names a city we do not have coords for, weather stays off rather than stamping SoFi because the Rams are listed home.
