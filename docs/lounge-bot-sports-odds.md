...
**Primetime Preview vs Publish:** Preview and Publish share `findPrimetimeGameCandidate`. Publish used to append `nfl` + `primetime` onto `community_feed_posts.category_pills`, which the Lounge allowlist rejects (`sports` only). Insert failed, Ops toasted “no game.” Fix: sanitize pills in `publishLoungeBotPost` and keep primetime on `sports`. Publish now matches Preview: **public Lounge tease + fan-only 4-desk Lounge post** (same dual-post model as slate) + VIP chat. Fan-only insert still needs Syndicate Connect + go live. Needs **`lounge-odds-poll`** redeploy on the project Ops invokes (sharpesyndicate.com → **prod**).
