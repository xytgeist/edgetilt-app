...
**Splits screenshot schedule (PT):** Scorecard banner + Splits paste checklist. Tue/Wed seed (full sides), Thu/Fri movers, Sat/Sun (NFL) or Fri/Sat (CFB) lock + totals tab. Same `syndicate_betting_splits` rows for Chedda (dog fire) and Tank (weighted street confirm/veto + totals). Bulk save deactivates **same source** only so Action + VSiN can both stay live. Desktop alert optional.
