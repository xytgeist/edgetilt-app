...
**ATS slate auto-publish (pg_cron):** CFB Fri **12pm PT** house (`cfb_slate_card`); Wed **2pm** VIP midweek Thu/Fri nights (`cfb_wed_midweek_vip`); Thu **3:30pm** CFB night tease (`cfb_thu_night_spotlight` … all-pass is **All pass**, not a fake home-number Lean); Sat **10am** CFB adds/kills (`cfb_sat_vip_adds_kills` … VIP only, silent unless a number walked **≥1.5 against the lock** or the locked side **crossed favorite/dog**; a still-dog away lock is not a kill). Sat **9am PT** UFC slate (`ufc_slate_card` … public + fan-only + VIP are **Scott + Rocco**; **next fight-night cluster only**, not the full MMA dump / Dec 31 futures / a same-day Euro card glued on; Tank round O/U parked until trained). Chedda and Tank are not printed. Fights where Scott and Rocco take the same side have that matchup line in gold and are listed first. Rocco independence check: `npm run syndicate:ufc-rocco-independence`. Contract: [`docs/syndicate-ufc-desk-contract.md`](./syndicate-ufc-desk-contract.md). NFL Fri **1pm** is a **lean** (`nfl_slate_card`), not a lock. Sat **7pm PT** steam confirm/kill (`nfl_sat_steam`). Sun **~8:30am PT** early-window inactives lock (`nfl_sunday_early_lock`, 1pm ET cluster). Sun **~11:30am PT** late-window lock (`nfl_sunday_late_lock`, 4:05/4:25 ET). SNF/TNF/MNF stay on primetime lean + 90-min lock. Migration `20260911190000`. Same Sat flip helper is on `nfl_sat_vip_adds_kills` (Ops leftover).
