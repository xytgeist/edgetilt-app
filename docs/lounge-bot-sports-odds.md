...
**Primetime Preview vs Publish:** Preview and Publish share `findPrimetimeGameCandidate`. Publish used to append `nfl` + `primetime` onto `community_feed_posts.category_pills`, which the Lounge allowlist rejects (`sports` only). Insert failed, Ops toasted “no game.” Fix: sanitize pills in `publishLoungeBotPost` and keep primetime on `sports`. Publish now matches Preview: **public Lounge 4-desk card + VIP chat** (same copy). No fan-only Lounge post. X gets the short lean. **`lounge-odds-poll` is on prod** (`jtjgtucumuoswnbauxry`, `dceefc49`) as of 2026-09-09.
