...
**Primetime Preview vs Publish:** Preview and Publish share `findPrimetimeGameCandidate`. Publish used to append `nfl` + `primetime` onto `community_feed_posts.category_pills`, which the Lounge allowlist rejects (`sports` only). Insert failed, Ops toasted “no game.” Fix: sanitize pills in `publishLoungeBotPost` and keep primetime on `sports`. Publish now matches Preview: **public Lounge + VIP chat + X** all get the same 4-desk card. No fan-only Lounge post. **`lounge-odds-poll` is on prod** (`jtjgtucumuoswnbauxry`) as of 2026-09-10.
