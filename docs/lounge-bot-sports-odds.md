...
**Primetime Preview vs Publish:** Preview and Publish share `findPrimetimeGameCandidate`. Publish used to append `nfl` + `primetime` onto `community_feed_posts.category_pills`, which the Lounge allowlist rejects (`sports` only). Insert failed, Ops toasted “no game.” Fix: sanitize pills in `publishLoungeBotPost` and keep primetime on `sports`. Publish now matches Preview: **public Lounge + VIP chat + X** all get the same 4-desk card. No fan-only Lounge post. Footer: lean not lock … desks stay as written; kill/flip only on a listed starter out or if the number walks off the edge; official lock is 90-min inactives. **Second drop** `nfl_primetime_lock` (~90 min pre): confirm the lean or kill (pass, cancel the ledger row). No auto-flip. Cron polls every 10 min in the Thu/Sun/Mon window; Edge only publishes inside 70-110 min pre-kick. Ops **Primetime lock** Preview/Publish can run anytime. **`lounge-odds-poll` is on prod** (`jtjgtucumuoswnbauxry`) as of 2026-09-10. Primetime cards still print the weather/stadium line. Location comes from Rundown `venue_location` / `venue_name`, not the listed home team's regular house. Melbourne / London / Mexico City / Munich / Frankfurt / Sao Paulo are seeded so Tank weather hits the actual site. If Rundown names a city we do not have coords for, weather stays off rather than stamping SoFi because the Rams are listed home.
...
**ATS slate auto-publish (pg_cron):** CFB Fri **12pm PT** house (`cfb_slate_card`); Wed **2pm** VIP midweek Thu/Fri nights (`cfb_wed_midweek_vip`); Thu **3:30pm** CFB night tease (`cfb_thu_night_spotlight`); Sat **10am** CFB adds/kills (`cfb_sat_vip_adds_kills` … VIP only, silent unless a number walked **≥1.5 against the lock** or the locked side **crossed favorite/dog**; a still-dog away lock is not a kill). Sat **9am PT** UFC slate (`ufc_slate_card` … public + fan-only + VIP are **Scott only**; **next fight-night cluster only**, not the full MMA dump / Dec 31 futures / a same-day Euro card glued on; Tank round O/U parked until trained). UFC desk rebuild is locked in [`docs/syndicate-ufc-desk-contract.md`](./syndicate-ufc-desk-contract.md) … do not code it until the independence test there is a real check. NFL Fri **1pm** is a **lean** (`nfl_slate_card`), not a lock. Sat **7pm PT** steam confirm/kill (`nfl_sat_steam`). Sun **~8:30am PT** early-window inactives lock (`nfl_sunday_early_lock`, 1pm ET cluster). Sun **~11:30am PT** late-window lock (`nfl_sunday_late_lock`, 4:05/4:25 ET). SNF/TNF/MNF stay on primetime lean + 90-min lock. Migration `20260911190000`. Same Sat flip helper is on `nfl_sat_vip_adds_kills` (Ops leftover).
...
**UFC / MMA 4-desk contract (locked, not coded):** [`docs/syndicate-ufc-desk-contract.md`](./syndicate-ufc-desk-contract.md). Data **(1)** landed. Live print is Scott only. Desk pick rules unchanged. Chedda PASS until a method feed.
