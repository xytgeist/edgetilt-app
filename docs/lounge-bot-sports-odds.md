...
**ATS slate auto-publish (pg_cron):** CFB Fri **12pm PT** house (`cfb_slate_card`); Wed **2pm** VIP midweek Thu/Fri nights (`cfb_wed_midweek_vip`); Thu **3:30pm** CFB night tease (`cfb_thu_night_spotlight`); Sat **10am** CFB adds/kills. NFL Fri **1pm** is a **lean** (`nfl_slate_card`), not a lock. Sat **7pm PT** steam confirm/kill (`nfl_sat_steam`). Sun **~8:30am PT** early-window inactives lock (`nfl_sunday_early_lock`, 1pm ET cluster). Sun **~11:30am PT** late-window lock (`nfl_sunday_late_lock`, 4:05/4:25 ET). SNF/TNF/MNF stay on primetime lean + 90-min lock. Migration `20260911190000`.
