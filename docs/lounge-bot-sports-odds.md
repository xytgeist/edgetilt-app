...
**Ops week calendar (PT):** Mon-Sun grid on **https://sharpesyndicate.com/ops** (above the desk tabs). Sport chips All / NFL / CFB / UFC. Status: Due / In / Missed / Later / Auto. Action/VSiN splits still read `syndicate_betting_splits` (shop week Tuesday). ESPN trench, PVAL/CFB/UFC checks, UFC slate, and Grade are mark-in for that shop week. Cron/GHA rows are labeled Auto so the week is visible even when Ryan does not have a paste. Click a cell to jump to Splits / Trenches / PVALs / CFB / UFC. Splits paste tab keeps the detailed Action/VSiN list. Desktop alert optional.
