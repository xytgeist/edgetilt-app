...
| Correcting a saved swap | Creator gets **Edit** on the saved swap card (unpaid only) → `poker_tournament_swap_update_terms` rewrites % + optional terms and re-runs `poker_tournament_swap_try_settle`. Edge-user counterparties must **re-accept** (`terms_reaccepted_at >= terms_revised_at`) before **Mark settled** is offered; guests auto-accept. Already-settled cash must be unwound first: **Unmark settled to edit** calls `poker_tournament_swap_mark_paid(id, false)`, reversing both personal bankroll posts and clearing **both** paid flags. SQL **`20260816130000`**. |
