...
| **Stable backing bankroll** (`poker_stable_backer_bankrolls`) | Backer capital + settle economics. **Never** `poker_bankroll_profiles`. First-time seed of slice capital (`20260809130000`, tightened **`20260810170000`** / **`20260810180000`**) only for **Create Stake initiator** or when the backer **Accepts** / capital debits … not on received pending invites, and never again after any allocation has been `bankroll_debited`. | Slice allocation debit; top-up debit / reduce credit; settle credit/debit (+ **Realized P/L** mirrors signed settle $); first-time seed adjustment. |
...
- **2026-08-11:** **Tournament close Commit review (client):** backer card hero is **Stake P/L** (rose when overall = stake P/L − markup fee is a loss); bullets are credit to backing bankroll + **Overall performance (markup applied)**; no baseline / “stake closes at” lines. Footnote: markup fees already applied. **`PokerStableCommitSyncPanel.jsx`**.
