...
- **Account delete (2026-09-15):** do **not** CASCADE away the deal or slice. Player / lead backer / slice backer `ON DELETE SET NULL` plus guest-label triggers (`20260915180000`). The other Edge party keeps the stake. The missing side is stamped **`{display name or handle} (Deleted User)`** from the profile before it is removed (`20260915190000`), not a generic deleted placeholder. Their personal bankroll / allocation rows still go with the deleted login. Same idea as tournament swaps (`20260915170000`). Copy invite / claim must **not** treat that stamp as a fresh guest (`20260916200000`).
...
| **Stable backing bankroll** (`poker_stable_backer_bankrolls`) | Backer capital + settle economics. **Never** `poker_bankroll_profiles`. On Create Stake (initiator) or Accept/debit, liquid is **auto topped up** by any shortfall vs paid stake amount (`20260811250000` … cash + tournament), logged as `auto_top_up` on the adjustments ledger (`20260811280000`). Hero **Backing Bankroll** still treats that credit as a loan (not owned cash) so a $704 book funding a $2500 slice shows **−$1796**, and **Portfolio** stays $704 (backing + MTM). Received pending invites still do not invent capital until Accept. | Slice allocation debit; top-up debit / reduce credit; settle credit/debit (+ **Realized P/L** mirrors signed settle $); deficiency top-up; full liquid ledger (deploy / release / close return / markup refund). |
...
- Roll reflects all package sessions (gross). **Buy-in, re-entries, and add-ons debit the stake roll as soon as they are logged** (active session), not only when the tournament is completed (`20260811230000`). Backer Stable **adds those in-play chips back** for Horse roll / stake MTM / portfolio while the session is still `active` (`1.4.270`).
...
| Where do swap settlements post? | **Player personal** bankroll on **Mark settled**. Each Edge user posts only their own signed, locally calculated amount; one player's action never writes the other's bankroll. Session “Your net” can show the local IOU earlier. `creator_bankroll_posted` / `counterparty_bankroll_posted` make each side idempotent. |
| Correcting a saved swap | Either Edge user gets **Edit** on their unpaid saved swap card. `poker_tournament_swap_update_my_book` changes only that viewer's % / optional terms and local settlement amount. The other player is notified but is never blocked; a neutral notice offers **Use their terms** or **Keep mine**. **Mark Unsettled** reverses only the viewer's own posted amount before editing (snapped in `*_bankroll_posted_amount` so later result sync cannot reverse the wrong dollars). `loadMyTournamentSwaps` overlays the viewer-local terms so history, details, totals, and settlement all agree with that viewer's books. Guests remain single-book until claim; claim catch-up posts only the claimer's side. SQL **`20260816140000`** + **`20260816150000`** supersede mandatory re-accept in **`20260816130000`**. |
...
| Optional swap terms (combinable) | **Default (no boxes):** extra bullets at face. Partner covers `pct × extra × face` when the extra-firer busts. % is of prize minus live extra-bullet face (and any face owed on the partner's busted extras). Do **not** subtract the first buy-in from prize. If both cash, extras only reduce the extra-firer's prize (no second face IOU). **Both must cash** voids unless both cashed. **Minimum cash threshold** voids unless either prize ≥ threshold. **Final bullet only** skips extras. **Final table only** activates if either finish is top 9 (or 6 if 6-max). Swaps are always **this bullet forward** (current + later series bullets only … already-fired Day 1s / earlier buy-ins excluded). SQL **`20260814140000`**–**`20260815120000`**. |
| Guest invite claim | Stake-style: email → `/poker-swap-claim?token=` review → create/sign in (invitation email) → `poker_tournament_swap_claim_link` / `_by_email` converts `guest` → Edge user → Bankroll Incoming (`tournamentSwap=`). Invite email must match the signed-in address exactly (Gmail dots are separate test accounts). A cancelled invite says cancelled, not swap not found. Confirm then attaches any other live guest swap for that exact email. SQL **`20260815140000`** + **`20260917120000`** + **`20260917130000`**. |
| Multi-flight series | Day 1A/B/C stay separate sessions (bankroll per flight). Same venue + buy-in + game + stripped name within 21 days is one event. A swap opened on 1B keeps picking up 1C / Day 2, and the active carried swap is visible on the next flight's Start Session (and new Log Past) form before that session is saved. When ending a tournament session with an active swap, a bust asks **Keep swap open** or **Close swap**. An in-the-money cash skips that prompt and closes the swap (you cannot buy back in after you cashed). Keeping it open records the session result/bullets but leaves that player's result unready and sends no result notification; closing marks that player's result ready and settles once both sides are ready. **Open swaps** on Bankroll stay hidden while the viewer still has a live tournament in that series (including the next Start Session after Keep swap open). The list returns only after that live session ends and the other side has not reported. If a player later manually enters the partner's result from completed-session Edit, the UI confirms that write and offers **Close my side** when the viewer's own result is still open. Day 2 is not a new bullet. **History UI** collapses same-series completed flights (same personal/stake ownership) into one card with aggregate invested/result/hours and deduped swaps; detail lists each flight with its own Edit. Soft-event UUID is **not** the group key. |
...
- **2026-09-16:** **Bankroll swipe debounce 50ms + live session title size:** carousel quiet-window is 50ms. Session detail game title is `text-3xl` / 36px line to match the type icon. Frontend **`1.4.281`** via **`test` → `main`**.
...
- **2026-09-16:** **Copy invite does not re-open deleted parties:** mint + claim-attach refuse **`Deleted User`** guest stamps so `20260914200000` cannot undo the account-delete preserve (`20260916200000`).
