...
- **2026-08-07:** **Periodic settle pay copy (client):** initiator review shows who pays whom inline before roll-reset ... backer sees `{player} pays you $X`; player sees `You pay {backer} $X` per slice. Hero credit is personal (player) or Stable backing bankroll (backer). **`PokerStablePeriodicSettleSheet.jsx`**.
