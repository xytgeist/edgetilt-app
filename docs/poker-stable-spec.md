...
| **Stable backing bankroll** (`poker_stable_backer_bankrolls`) | Backer capital + settle economics. **Never** `poker_bankroll_profiles`. First open stake with **$0** / missing row auto-seeds slice capital (`20260809130000`) so Accept / Create Stake does not start negative. | Slice allocation debit; top-up debit / reduce credit; settle credit/debit (+ **Realized P/L** mirrors signed settle $); first-time seed adjustment. |
...
- **2026-08-10:** **Pending offer withdrawn (SQL + client):** `poker_stable_cancel_stake_deal` rewrites Edge invite/nudge Alerts to **`poker_stable_offer_withdrawn`** (in place, no new push; resurfaces unread). Stable deep link to a missing deal shows **This stake offer was withdrawn.** Migration **`20260810120000`**.
