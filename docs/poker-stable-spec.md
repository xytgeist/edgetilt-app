...
| **Stable backing bankroll** (`poker_stable_backer_bankrolls`) | Backer capital + settle economics. **Never** `poker_bankroll_profiles`. On Create Stake (initiator) or Accept/debit, liquid is **auto topped up** by any shortfall vs paid stake amount (`20260811250000` … cash + tournament), logged as a manual adjustment. Received pending invites still do not invent capital until Accept. | Slice allocation debit; top-up debit / reduce credit; settle credit/debit (+ **Realized P/L** mirrors signed settle $); deficiency top-up adjustment. |
