...
- **2026-08-07:** **Periodic settle pay copy (client):** initiator review lists who pays whom as bullets under the credit hero, then **Stake roll resets to $X and the stake remains open**. Backer: `{player} pays you $X`; player: `You pay {backer} $X` per slice. Hero credit is personal (player) or Stable backing bankroll (backer). **`PokerStablePeriodicSettleSheet.jsx`**.
