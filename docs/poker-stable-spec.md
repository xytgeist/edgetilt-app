...
| Guest invite claim | Stake-style: email → `/poker-swap-claim?token=` review → create/sign in (invitation email) → `poker_tournament_swap_claim_link` / `_by_email` converts `guest` → Edge user → Bankroll Incoming (`tournamentSwap=`). No anonymous result form. Accept / Decline / bind / start session stays on Bankroll. **Invite via text:** one guest closes the share sheet after Share; several guests keep it open until each is shared (or Done). SQL **`20260815140000`**. |
