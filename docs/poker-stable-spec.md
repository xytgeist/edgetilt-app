...
| Guest invite claim | Stake-style: email → `/poker-swap-claim?token=` review → create/sign in (invitation email) → `poker_tournament_swap_claim_link` / `_by_email` converts `guest` → Edge user → Bankroll Incoming (`tournamentSwap=`). Gmail / googlemail treat dots and plus-tags as the same mailbox. A cancelled invite says cancelled, not swap not found; confirm then attaches any other live guest swap for that mailbox. SQL **`20260815140000`** + **`20260917120000`**. |
