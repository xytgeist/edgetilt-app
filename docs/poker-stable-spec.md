...
- **2026-08-04:** **Settle commit review breakdown (client):** player periodic/close settle review mirrors **`PokerStablePeriodicSettleSheet`** / close sheet (profit above baseline, personal bankroll credit, stake card now vs after commit) ... not per-slice backer accounting. **`PokerStableSettleCommitBreakdown.jsx`**; export **`loadSettlementBundle`** from **`pokerStableApi.js`**.
