...
- **2026-08-04:** **Settle commit review breakdown (client):** **`PokerStableCommitSyncModal`** loads settlement snapshot + slice lines and shows roll/baseline, profit above baseline, per-backer shares, personal bankroll credit, and post-commit roll/baseline when a stakee reviews a periodic/close settle. **`PokerStableSettleCommitBreakdown.jsx`**; export **`loadSettlementBundle`** from **`pokerStableApi.js`**.
