...
- **2026-08-04:** **Settle commit review (client):** player periodic/close review modal shows **Personal bankroll on commit** (+ roll/baseline context). Fixed broken deal load (`makeup_enabled` column does not exist). **`PokerStableCommitSyncModal.jsx`**; export **`loadSettlementBundle`** from **`pokerStableApi.js`**.
