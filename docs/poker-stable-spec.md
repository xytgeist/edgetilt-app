...
- **Account delete (2026-09-15):** do **not** CASCADE away the deal or slice. Player / lead backer / slice backer `ON DELETE SET NULL` plus guest-label triggers (`20260915180000`). The other Edge party keeps the stake. The missing side is stamped **`{display name or handle} (Deleted User)`** from the profile before it is removed (`20260915190000`), not a generic deleted placeholder. Their personal bankroll / allocation rows still go with the deleted login. Same idea as tournament swaps (`20260915170000`).
