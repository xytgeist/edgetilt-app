...
| Guest invite claim | Stake-style: email → `/poker-swap-claim?token=` review → create/sign in (invitation email) → `poker_tournament_swap_claim_link` / `_by_email` converts `guest` → Edge user → Bankroll Incoming (`tournamentSwap=`). No anonymous result form. Accept / Decline / bind / start session stays on Bankroll. SQL **`20260815140000`**. |
