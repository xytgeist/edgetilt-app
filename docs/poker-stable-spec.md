...
| **Stable backing bankroll** (`poker_stable_backer_bankrolls`) | Backer capital + settle economics. **Never** `poker_bankroll_profiles`. First-time seed of slice capital (`20260809130000`, tightened **`20260810170000`**) only for **Create Stake initiator** or when the backer **Accepts** / capital debits … not on received pending invites. | Slice allocation debit; top-up debit / reduce credit; settle credit/debit (+ **Realized P/L** mirrors signed settle $); first-time seed adjustment. |
