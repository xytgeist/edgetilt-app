...
- [ ] **Delete account GoTrue grants (2026-09-15):** **Not applied on prod.** SQL **`20260915160000_auth_admin_delete_user_grants.sql`** lets **`supabase_auth_admin`** CASCADE/SET NULL into `public.*` during `admin.deleteUser`. Applied **test**; throwaway delete succeeded. Wait for Ryan. `apply-migration-once.mjs --target=production`. No Edge redeploy. No FK changes.
