...
- [ ] **Poker swap terms (test only until Ryan asks):** apply **`20260814140000_poker_tournament_swap_terms.sql`** then **`20260814150000_poker_tournament_swap_extra_bullet_face.sql`** on prod before promoting the client. **No Edge redeploy.** Smoke: default extras-at-face (3 bullets vs 1, both bust, 10% → $200); both-must-cash busts settle $0; final-bullet skips extras; final-table waits for finish place then voids if neither made 9 (or 6 if 6-max).
