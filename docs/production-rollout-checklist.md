...
- [ ] **Open swaps on Bankroll (2026-09-16):** frontend **`1.4.254`** on **`test`**. **Open swaps** list on Poker Bankroll. Tap opens session card or standalone sheet. Redeployed **`poker-tournament-swap-notify`** (`kind=nudge`) on **test**. **Prod Edge + `main`:** Mac access token 403 on `jtjgtucumuoswnbauxry` functions deploy; promote when Ryan asks.
