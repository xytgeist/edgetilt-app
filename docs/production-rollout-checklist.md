...
- [ ] **Poker Stable stakee hide archived stake (pending promote):** apply **`20260816160000`** on **`jtjgtucumuoswnbauxry`**, then frontend via **`test` → `main`**. **No Edge redeploy.** Smoke: Bankroll → Archive → Delete archived stake → **Keep sessions** leaves merged personal history/metrics; **Delete sessions** removes those personal rows/metrics; backer Closed history and settled bankroll balances remain unchanged.
