...
- [x] **Open swaps on Bankroll (2026-09-16):** frontend **`1.4.254`** via **`test` → `main`**. **Open swaps** list on Poker Bankroll. Tap opens session card or standalone sheet. Redeployed **`poker-tournament-swap-notify`** (`kind=nudge`) on **test**. **Prod Edge still open:** Mac access token 403 on `jtjgtucumuoswnbauxry` functions deploy.
