...
- [ ] **Poker live session pause (2026-08-14):** apply **`20260814120000_poker_bankroll_session_pause.sql`** on **`jtjgtucumuoswnbauxry`** before promoting the client (`paused_at` / `paused_seconds` on **`poker_bankroll_sessions`**). **No Edge redeploy.** Frontend: Vercel **`edgetilt.com`** via **`test` → `main`** only after SQL. Smoke: start cash or tourney → pause next to the timer → clock freezes + header **Session paused** → resume → End Session hours exclude the break.
