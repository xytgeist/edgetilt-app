...
- [x] **Poker tournament swaps (2026-07-30 promote, `main` @ `44ba6b13`):** apply in order on **`jtjgtucumuoswnbauxry`** — **`20260730140000`** (soft events + swaps + claim tokens/RPCs), **`20260730150000`** (`poker_tournament_swap` activity), **`20260730160000`** (`poker_tournament_swap_result` + `detail_text` / swap id on activity page), **`20260730170000`** (Realtime on swaps), **`20260730180000`** (`last_activity_at` soft-event window). Redeploy **`poker-tournament-swap-notify`** + **`lounge-send-activity-push`**. Frontend: Vercel **`edgetilt.com`** via **`test` → `main`**. Secrets: **`PUBLIC_APP_URL=https://edgetilt.com`**; guest SMS needs **`TWILIO_*`** (API key preferred) + From number when ready; Resend already used for email. Smoke: Start/Log tourney → add Edge/guest swap → Send swap on live session → Incoming Accept / claim link → end session syncs your result → Mark settled when IOU ≠ 0; session cards show **Swaps** + Settled `(+$x)` / `(-$x)`.
...
_Last updated: **2026-07-30** — Poker tournament swaps promote **`main` @ `44ba6b13`** (SQL **`20260730140000`–`180000`**, Edge **`poker-tournament-swap-notify`** + **`lounge-send-activity-push`**). Earlier **2026-07-29**: Casino LV geo gaps **`20260730120000`**; Poker Bankroll follow-up **`6c055e17`**; Poker Bankroll Manager **`20260729000000`–`20260730030000`**. Prior **2026-07-25**: Lounge staged video (`3f48b328`)._
