...
- [ ] **EdgeiOS APNs (after test smoke):** apply **`20260825210000_apns_device_tokens.sql`**. Set Edge secrets **`APNS_KEY_ID`**, **`APNS_P8`** (Auth Key PEM), optional **`APNS_TEAM_ID`** / **`APNS_BUNDLE_ID`**. Redeploy **`lounge-send-activity-push`**, **`send-test-push`**, **`send-due-offer-reminders`**. Do not apply until Ryan asks. Tap deep links still need Mac `didReceive`.
