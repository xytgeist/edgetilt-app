...
- [ ] **Poker swap terms (test only until Ryan asks):** apply **`20260814140000_poker_tournament_swap_terms.sql`** on prod before promoting the client that writes `both_must_cash` / `final_bullet_only` / `final_table_only`. **No Edge redeploy.** Smoke: draft checkboxes stack; both-must-cash busts settle $0; final-bullet uses one buy-in; final-table waits for finish place then voids if neither made 9 (or 6 if 6-max).
