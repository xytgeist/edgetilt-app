...
- [x] **Chat new-member history toggle (2026-09-04):** **`20260904180000_chat_new_member_history.sql`** on **`jtjgtucumuoswnbauxry`**. Owner Privacy switch **Show chat history to new members**; join-time `history_starts_at`; message RPCs + RLS. **No Edge redeploy.** Frontend via **`test` → `main`**. **Ryan smoke PASSED 2026-09-04.**
- [x] **Chat members-can-post (2026-09-04):** **`20260904200000_chat_members_can_post.sql`** on **`jtjgtucumuoswnbauxry`**. Owner **Allow members to post**; insert trigger + **`lounge-chat`**. Redeploy **`lounge-chat`**. Frontend via **`test` → `main`**. **Ryan smoke PASSED 2026-09-04.**
...
- [x] **Casino downtown Fremont GPS (2026-09-07):** **`20260907200000_casinos_lv_downtown_geo.sql`** applied on **test** + **`jtjgtucumuoswnbauxry`** ... Circa (fill existing nameless-geo row) + Plaza Hotel & Casino / The D / Four Queens / Golden Gate Hotel & Casino. **No Edge / frontend.** Smoke: GPS on Fremont, Near you ranks Circa / Plaza / The D / Four Queens / Golden Gate.
- [x] **Play Logbook Edit Play game (2026-09-07 promote):** **`20260907210000_play_log_shared_session_template.sql`** applied on **`jtjgtucumuoswnbauxry`**. Frontend via **`test` → `main`**. Solo game change writes `play_log_entries.template_id`. Shared game change uses **`play_log_set_shared_session_template`**. Smoke: Edit Play → change game → overlapping fields kept → Cancel left of Save → Save; partnered play updates both partner entries.
- [x] **Lounge @mention typeahead (2026-09-07 promote):** frontend-only **`test` → `main`**. Display name match + picker closes after pick (`@handle `). **No SQL / Edge.** Smoke: `@MeSoSl` finds MeSoSlotty; tap `@KennyNorman` → picker stays closed.
...
- [x] **iPhone Lounge/chat video (2026-09-22 promote):** no new SQL. Redeploy **`lounge-cf-stream-tus-create`**, **`lounge-cf-stream-direct-upload`**, **`lounge-chat-r2-video-upload`** on **`jtjgtucumuoswnbauxry`**. Frontend via **`test` → `main`**. Web **`1.4.484`**. Free video **2:20 / 512 MB**, Edge Pro **20 min / 8 GB** (chat stays on R2, stored file **5 GB**). IPA picks, exports, and uploads. Lock screen keeps a started encode going on iOS 26+. Next clip encodes while the previous one uploads. Store marketing still **1.4.95**. Smoke: post a long clip on the new IPA, lock during encode, post a second clip while the first is uploading.
- [x] **Prod full Edge + SQL catch-up (2026-09-24):** redeployed **all 61** repo Edge Functions to **`jtjgtucumuoswnbauxry`** (scoreboard splits wiring, fantasy, new `stripe-ensure-edge-pro-price`). Applied/recorded product migrations that were test-recorded only (`20260711140000`, offer-reminder cron, `app_section_visits` chain, Edge Pro reply gate, poker claim email finals). Skipped theo mailbox / trench followup (test-only). Frontend already on **`main`** **`1.4.562`**. Smoke: hub hero public bets/$ on any Sunday game with Action paste (prod has full slate).
- [x] **Game Hub jersey + halftime flip (2026-09-26, `main` @ `f28f0351`):** applied **`20260926013000_nfl_players_jersey`** on **`jtjgtucumuoswnbauxry`**; synced Sleeper jerseys (**873** filled). Deployed **`lounge-nfl-game-fantasy`**. Frontend via **`test` → `main`**. Vercel prod = web **`1.4.691`**. Smoke: edgetilt.com hub rush/catch paints `#N` from roster or PBP; endzones flip after HT.
- [x] **Game Hub field anim + CFB scoreboard catch-up (2026-09-26, `main` @ `1495f181`):** no new SQL. Frontend via **`test` → `main`**. Vercel prod = web **`1.4.689`**. Redeployed **`lounge-sports-scoreboard`** on **`jtjgtucumuoswnbauxry`**. Smoke: edgetilt.com Lounge pill → hub completed pass/rush Tap to replay + TD celebrate; CFB slate logos match abbrev (not mascot).
...
- [x] **NFL game hub Players / Fantasy stack (2026-09-24 promote):** applied **`20260921210000`** (record), **`20260923160000_nfl_players`**, **`20260924010000`** depth, **`20260924010100`** injury on **`jtjgtucumuoswnbauxry`**. Synced **904** Sleeper fantasy seats (incl. **69** K/DEF) + ESPN ids; headshot URLs → shared R2 (`media-test.lvslotpro.com/sports/nfl/players/…`). Deployed **`lounge-nfl-game-fantasy`** + redeployed **`lounge-sports-scoreboard`**. Schedule: GHA **`.github/workflows/nfl-players-roster-sync.yml`** Tue/Fri **15:00 UTC** (test then prod). Frontend via **`test` → `main`** (**`1.4.531`**). Smoke: edgetilt.com Lounge pill → hub Players / Fantasy / Stats ladders / injury pills / K·DEF H2H.
...
- [x] **Anon-execute lockdown, stage 2 (2026-08-27):** applied **`20260827180000`** on **test then prod**, recorded on both. **No Edge redeploy, no frontend change.** Completes the full database sweep of all 254 remaining `anon`-executable functions: 124 client RPCs locked to `authenticated` only, 114 internal helpers fully revoked from `anon`+`authenticated` (retaining `service_role` and owner execution), and exactly the 16 legitimate public guest/referral/policy RPCs preserved. Verified with `has_function_privilege('anon', oid, 'execute')` on prod: exactly 16 functions remain public.
- [x] **Poker Stable detach-sessions RPC guarded then taken internal (2026-08-27):** applied **`20260827160000`** then **`20260827170000`** on **`jtjgtucumuoswnbauxry`** via **`apply-migration-once.mjs`** (both recorded). **No Edge redeploy.** Frontend rides **`test` → `main`** whenever you next promote … **DB may lead web here**, see below. Fixes the one authorization bug the anon-execute audit found that grants could not: `poker_stable_detach_stake_sessions_to_personal` took a bare `p_deal_id` with **no caller check**, so any logged-in member could push any deal's sessions onto the stakee's personal bankroll and move `poker_bankroll_profiles.overall_bankroll`. **⚠️ Do not guard this on "caller is the stakee"** … `poker_stable_decline_backer_slice` and the `poker_stable_after_backer_exit_detach` trigger both arrive with the **backer** as caller; guard on participation (`poker_stable_user_can_access_deal`) and gate on `auth.uid() is not null` so trigger/service paths keep working. `20260827170000` then drops `authenticated` because the browser call was redundant (the status→`revoked` trigger already detaches). **Safe with old web live:** `supabase.rpc()` returns `{ error }` instead of throwing and the result is discarded, so pre-promote `main` silently ignores the denial. Smoke: revoke a deal whose backers have all declined → sessions land on personal bankroll + `sessions_detached` ledger line.
...
- [x] **Slots Edge Pro + Lifetime grant Edge Pro (2026-09-07):** applied **`20260906120000`** then **`20260907120000`** on **`jtjgtucumuoswnbauxry`**. Frontend via **`test` → `main`**. Starter stays out.
- [x] **Store-screenshot hide (2026-09-07, `main` @ `51561730`):** cherry-pick only, not a full `test` FF. AP Guide card Delete + Slots Edge lock, Play Logbook **Primary game templates** off. **No SQL / Edge.** Wait for Vercel **edgetilt.com**. Prod SQL **`20260906120000`** + **`20260907120000`** applied; frontend via **`test` → `main`**.
- [x] **Apple IAP dual-path + ASSN (2026-09-05 promote):** applied **`20260905120000`** then **`20260905140000`** on **`jtjgtucumuoswnbauxry`** via **`apply-migration-once.mjs`**. Redeployed **`apple-iap-verify`** + **`apple-iap-notify`**. Probe: verify **401** session (alive), notify handler runs (not gateway 404). Existing Stripe `user_subscriptions` unchanged (`billing_provider` default). Frontend: Vercel **`edgetilt.com`** via **`test` → `main`**. **Ryan:** paste Production ASSN V2 URL `https://jtjgtucumuoswnbauxry.supabase.co/functions/v1/apple-iap-notify`. Do not Add for Review. Smoke: web entitlements still unlock; Restore proves verify without a real charge.
- [x] **IPA Lifetime card (2026-09-14):** Apple approved higher price points. Subscribe carousel was Starter + Pro + Lifetime on the shell (`1.4.210`). ASC Lifetime IAP `6809032089` list is **`$1,499.99`** US (founding window; was `$1,999.00`). Offer-code path parked. **Superseded 2026-09-23:** Review **2.1(b)** rejected because Lifetime was visible without the IAP in the submission. Shell hides Lifetime again (`1.4.494`); do not attach Lifetime IAP on the next resubmit.
- [x] **Lounge UGC reports (2026-09-11):** applied **`20260911130000_lounge_ugc_reports.sql`** on **prod** via **`apply-migration-once.mjs`**. Recorded. Frontend **`1.4.173`** promoted `test` → `main`. No Edge deploy.
- [x] **Swipe debounce 50ms + session title (2026-09-16):** frontend **`1.4.281`** via **`test` → `main`**. Carousel quiet-window 50ms. Session detail title matches the type icon. **No SQL / Edge.**
- [x] **Bankroll carousel ledger follows swipe (2026-09-16):** frontend **`1.4.280`** via **`test` → `main`**. Session list follows the centered hero card. **No SQL / Edge.**
- [x] **0% player profit split (2026-09-16):** frontend **`1.4.279`** via **`test` → `main`**. Player can take 0% of sold action. **No SQL / Edge.**
- [x] **Live session backers terms (2026-09-16):** frontend **`1.4.278`** via **`test` → `main`**. Live Sell Action session detail shows action %, backing $, and profit split. **No SQL / Edge.**
- [x] **Guest claim OAuth matching reverted (2026-09-16):** **`20260917300000`** was test-only, then **`20260917310000`** restored exact invite email. **Do not apply `20260917300000` on prod.** **No Edge.**
- [x] **Sell Action +Edge user picker (2026-09-16):** frontend **`1.4.271`** via **`test` → `main`**. Tapping an Edge user on Start Session Sell Action adds the backer draft. **No SQL / Edge.**
- [x] **Stable live-session MTM (2026-09-16):** frontend **`1.4.270`** via **`test` → `main`**. Horse roll / stake MTM restore in-play chips while the session is open. **No SQL / Edge.**
- [x] **Sell Action guest text invite (2026-09-16):** SQL **`20260917220000`** on **test + prod**. Frontend **`1.4.268`** via **`test` → `main`**. Mint works on unclaimed active guest backer slices. **No Edge redeploy.**
- [x] **Tournament picker dates + Edge avatars (2026-09-16):** frontend **`1.4.267`** via **`test` → `main`**. Start Session labels include the date. Partner / +Edge user picker shows avatars (guests stay text-only). **No SQL / Edge.**
- [x] **Grade-picks settle (2026-09-05):** SQL **`20260905130000`** on test + prod. Redeploy **`lounge-odds-poll`** + **`lounge-odds-ingest`**. One Edge call, 150s pg_net, cron **:22** PT. Smoke: pending Thu/Fri CFB leave `pending` after invoke. Frontend version **`1.4.61`** (web only).
- [x] **Ping / Share invite placement (2026-09-16):** frontend **`1.4.256`** via **`test` → `main`**. Ping hidden while live. Copy text left, Share right. **Share invite** only on ongoing session cards. **No Edge redeploy.**
- [x] **ITM skip swap prompt + history swap line (2026-09-16):** frontend **`1.4.259`** via **`test` → `main`**. Cash ITM closes the swap (no Keep swap open). History cards show name + % only. **No Edge redeploy.**
- [ ] **Cancelled swap claim copy + exact email (2026-09-16):** SQL **`20260917120000`** then **`20260917130000`** on **test** (Gmail-dot merge reverted). Frontend **`1.4.263`** on **`test`**. Apply both on **prod** when Ryan asks. **No Edge redeploy.**
- [x] **Multi-guest text invite share sheet (2026-09-16):** frontend **`1.4.261`** via **`test` → `main`**. Share sheet stays open until each guest is shared. **No SQL / Edge.**
- [x] **Open swaps wait for live series to end (2026-09-16):** frontend **`1.4.260`** via **`test` → `main`**. Hide Open swaps while a series-matching live session exists. Start Session prepends the live row. **No SQL / Edge.**
- [x] **Guest text invite + Open swaps after end (2026-09-16):** frontend **`1.4.255`** via **`test` → `main`**. Guest **or invite via text** checkbox. Share is the green CTA and closes the sheet. Open swaps wait until the associated session has ended. **No Edge redeploy.**
- [x] **Open swaps on Bankroll (2026-09-16):** frontend **`1.4.254`** via **`test` → `main`**. **Open swaps** list on Poker Bankroll. Tap opens session card or standalone sheet. Redeployed **`poker-tournament-swap-notify`** (`kind=nudge`) on **test + prod**.
- [x] **Guest copy-invite mint (2026-09-16):** applied **`20260914200000`** then **`20260916200000`** on **`jtjgtucumuoswnbauxry`** via **`apply-migration-once.mjs`**. Mint RPCs only; delete-account preserve stays. Deleted-party guest stamps cannot mint or re-claim. Frontend **`1.4.252`** via **`test` → `main`**. **No Edge redeploy.**
- [x] **Delete account GoTrue grants + swap/stake preserve (2026-09-15):** applied **`20260915160000`** through **`20260915200000`** on **`jtjgtucumuoswnbauxry`** via **`apply-migration-once.mjs`**. Redeployed **`delete-own-account`**. Stake/swap party FKs are SET NULL + **`Name (Deleted User)`**. Test delete already PASSED (Ryan, lvslotpro). Prod smoke: Delete on **edgetilt.com** when Ryan wants.
- [x] **App Review IPA after 125 reject (2026-09-15):** Ryan asked. **`test` → `main`** (`0ac76f5c`). **1.4.95 (20260915213919)** uploaded to ASC. Not 125. Attach + Submit for Review after processing. CallKit off on `CHN`. China stays listed. SIWA is IPA-only.
- [ ] **UFC aliases + card facts (2026-09-12, `main` @ `8f44c549`):** frontend is on Vercel. **Not applied on prod:** SQL **`20260913020000_ufc_card_facts_and_aliases.sql`**. **Not redeployed:** **`lounge-odds-poll`** (aliases, slate filter, Tank UFC totals park, Scott-only UFC print). Wait for Ryan. Dashboard SQL Editor (multi-statement) or `apply-migration-once.mjs --target=production`. Then record `schema_migrations`.
...
- [x] **Play Logbook LEDGER (2026-09-08 promote):** applied in order on **`jtjgtucumuoswnbauxry`** via **`apply-migration-once.mjs`** — **`20260907220000`** (settlements table), **`20260907230000`** (independent-books Alert `play_log_ledger_settled`), **`20260907240000`** (Remain Unsettled), **`20260907250000`** (one-play paid Alert), **`20260907260000`** (Nudge). Base Logbook through **`20260907210000`** already on prod. Redeployed **`lounge-send-activity-push`**. Frontend: Vercel **`edgetilt.com`** via **`test` → `main`**. **`1.4.119`.** Smoke: LEDGER Settle All / Paid → partner Update my books; Remain Unsettled; Nudge Alert + 1h cooldown; like still inserts.
- [x] **Alerts missing `navigateToGuideSlug` (2026-09-08):** frontend **`test` → `main`**. Restored Alerts imports dropped in the one-play paid Alert commit. **No SQL / Edge.** **`1.4.123`.**
- [x] **FAB Alerts from Slots/Poker (2026-09-08):** frontend **`test` → `main`**. Row tap from off-Lounge FAB closes the panel and switches home so keep-alive Lounge is not `hidden`/`inert`. **No SQL / Edge.** **`1.4.124`.**
- [x] **Play Logbook follow-up (2026-09-08 promote):** frontend-only **`test` → `main`**. **No SQL / Edge.** Partner can Paid themselves (manager gets Update my books); Partners (i) light-mode contrast; ANALYZE Scope is logged games only. **`1.4.122`.**
...
# Phone login (Send SMS hook). Deployed on test and production. Phone provider + hook on. SMS autoconfirm off.
