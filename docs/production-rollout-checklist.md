...
- [ ] **Poker swap Min cash term:** apply **`20260815120000`** on **`jtjgtucumuoswnbauxry`** (column + settle RPC + claim preview). **No Edge redeploy.** Frontend via **`test` → `main`**. Smoke: threshold met by one side settles; neither hits → $0; exact threshold activates; label shows **Min cash $…**.
