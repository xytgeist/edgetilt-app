...
- [ ] **Poker session piece stake + live pause (2026-08-14):** apply on **`jtjgtucumuoswnbauxry`** before promoting the client … **`20260814120000_poker_bankroll_session_pause.sql`** then **`20260814130000_poker_stable_piece_settle.sql`**. **No Edge redeploy.** Frontend via **`test` → `main`** only after both. Smoke: personal Start Session → Backers → guest 50% → start → End Session → stake gone from carousel; pause still works on the live card.
