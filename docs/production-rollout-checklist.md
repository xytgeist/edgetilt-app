...
- [x] **UFC metrics what-changed dump (2026-09-18):** applied **`20260918120000_ufc_metrics_sync_runs.sql`** on **test + prod** (empty table, public select, service_role write). Recorded in `schema_migrations`. **No Edge redeploy.** Frontend **`1.4.289`** on **`test`**. First row lands on the next Tuesday GHA (`syndicate:sync-ufc-metrics`). Smoke: Ops → UFC Metrics → "Last UFC Stats refresh" says no dump yet (not a missing-table error).
