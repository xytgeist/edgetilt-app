...
- [x] **Chat new-member history toggle (2026-09-04):** **`20260904180000_chat_new_member_history.sql`** on **`jtjgtucumuoswnbauxry`**. Owner Privacy switch **Show chat history to new members**; join-time `history_starts_at`; message RPCs + RLS. **No Edge redeploy.** Frontend via **`test` → `main`**.
- [x] **Chat members-can-post (2026-09-04):** **`20260904200000_chat_members_can_post.sql`** on **`jtjgtucumuoswnbauxry`**. Owner **Allow members to post**; insert trigger + **`lounge-chat`**. Redeploy **`lounge-chat`**. Frontend via **`test` → `main`**.
