...
- [ ] **Poker swap independent books (test only, 2026-08-16):** when Ryan promotes, apply **`20260816130000`** then **`20260816140000`** on **`jtjgtucumuoswnbauxry`**, redeploy **`poker-tournament-swap-notify`**, then promote frontend. `140000` supersedes the mandatory re-accept model from `130000`: each Edge player edits, calculates, settles, and unsets only their own books / personal-bankroll entry; the other player receives an informational revision Alert with optional **Use their terms** / **Keep mine**. Smoke with two accounts: A edits 5% → 10%; B stays 5% and can settle; A settles only A bankroll; B chooses Keep mine and settles only B bankroll.
