...
- [ ] **Sell Action +Edge user picker (2026-09-16):** frontend **`1.4.271`** on **`test`**. Tapping an Edge user on Start Session Sell Action adds the backer draft. **No SQL / Edge.** Promote with `test` → `main` when Ryan asks.
- [ ] **Stable live-session MTM (2026-09-16):** frontend **`1.4.270`** on **`test`**. Horse roll / stake MTM restore in-play chips while the session is open. **No SQL / Edge.** Promote with `test` → `main` when Ryan asks.
