...
- [ ] **Play Logbook Edit Play game (`20260907210000`):** **`play_log_set_shared_session_template`** on **`jtjgtucumuoswnbauxry`** when promoting. Frontend via **`test` → `main`**. Solo game change works from client `play_log_entries.template_id`. Shared game change needs this RPC. Smoke: Edit Play → change game → overlapping fields kept → Save; partnered play updates both partner entries.
