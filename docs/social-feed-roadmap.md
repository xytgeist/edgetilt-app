...
- **Product:** Feed composer **Start thread** → full-screen **`LoungeThreadComposeSheet`**. Pro composer plus-square stays on the markdown screen and appends caption fields (media on part 1); markdown bar stays pinned while captions scroll. Parts 2+ insert as **`feed_comments`** with **`is_thread_part`** (full like/repost/bookmark/reply per section); feed shows one root card + **Thread · N parts** badge.
