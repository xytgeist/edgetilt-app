...
- **Shipped (test):** Unified **Stream + image/GIF lightbox** via **`LoungeStreamLightboxContext`** (feed, detail, comments, profile); image **pinch-to-zoom + pan** (`loungeLightboxImageZoom.js`); Stream hero expand shares interaction bar + repost menus with still lightboxes. **Multi-image lightbox pager** uses **scroll-snap paging** (`useLoungeLightboxCarouselSnap.js`) — distinct from feed carousel free-scroll + axis lock.
- **Shipped (test, Jul 2026):** **Horizontal image carousels** on feed, post detail, comment rows, and profile reply embeds — **`loungeFeedImageAttachment.js`** full-bleed rules; thread hierarchy limits bleed to focus row only; post detail + profile scroll containers **`overflow-x-hidden`** to block **`100vw`** bleed pan. **Quote repost inset** shows compact OP avatar beside meta row (**`LoungeQuoteRepostEmbedAuthorMeta`**).
...
- **Shipped (test):** **Cloudflare Stream** — Edge **`lounge-cf-stream-direct-upload`** / tus-create; client **`src/utils/loungeVideoUpload.js`** resumable tus (`findPreviousUploads` + soft-pause on tab hide / resume on visible), HLS poll until ready; DB **`community_feed_posts.stream_video_uid`**. Playback **`LoungePostStreamVideo.jsx`**. Video bytes **not** in Supabase Storage. Cap **2:20 / 512 MB** free, **20 min / 8 GB** Edge Pro.
...
- **Product:** Feed composer **Start thread** → full-screen **`LoungeThreadComposeSheet`**. Pro composer plus-square stays on the markdown screen and appends caption fields (media on part 1); markdown bar stays pinned while captions scroll. Parts 2+ insert as **`feed_comments`** with **`is_thread_part`** (full like/repost/bookmark/reply per section); feed shows one root card + **Thread · N parts** badge. Profile **Replies** tab excludes `is_thread_part` rows and keeps reply captions beside the avatar (not post-detail full-width focus layout).
...
- **Post-detail reply delivery (2026-08-16):** Replies paint an author-only optimistic row immediately, keep the detail screen interactive, and reconcile the temporary row after persistence. The insert uses a client-generated UUID with four transient-failure attempts, so a committed request whose response was lost cannot produce a duplicate. Final failure removes the optimistic row, saves the caption per user + post for seven days, and shows a tappable toast that reopens the post/thread with the caption restored in the composer.
...
- **Shipped (foreground in-app toast, test build 2026-05-23; iOS visibility harden 2026-08-12):** **`push-sw.js`** diverts activity pushes (Lounge + Chat + Poker Stable/etc.) to **`lounge-activity-inapp`** → **`AppShell`** banner when the app is **visibly open** (Cache beacon / `visibilityState` / probe ... not `client.focused` alone; iPhone PWA). Background/minimized → OS push. Offers calendar pushes unchanged. **`LoungeActivityInAppToast.jsx`**; badge refreshes on **`lounge-activity-arrived`**. Ryan smoke **§21d** **PASSED** @ **`dcc3852`** (per-tap mark read).
- **Shipped (push tap opens the actual reply, test build 2026-08-16):** Edge deep links for comment-ish alerts (`comment_on_post`, `reply_to_comment`, `mention_in_comment`, comment `like` / `bookmark` … including grouped batches) now carry **`comment=`** alongside `post=`, and **`push-sw.js`** repeats `postId` / `commentId` in the `app-navigate` message for clients that never reload the URL. Landing on the feed from an alert triggers a **silent feed head refresh** plus the same open path as an Alerts row (**`onLoungeOpenPostFromNotifications`** → **`openDirectCommentPostDetail`**), and re-entering a post that is already open **refetches comments** (the load effect is keyed on post id, so the notifying reply used to be missing). URL ownership rule: when `post=` is already in the URL the Lounge deep-link effect opens it … `AppShell` does not also hand it to SocialFeed. **Redeploy `lounge-send-activity-push`** and force-close the PWA to pick up the new service worker.
...
- **Shipped:** counsel-reviewed policy text for **Digiverse Ventures, LLC** (Wyoming) ... Ryan sign-off **2026-07-01**; operating entity name in **`legalPolicyVersion.js`** updated **2026-07-23**. **10DLC SMS** exact TCR `originator optin` language on Privacy **§5** + **§6** (**`LEGAL_POLICY_VERSION` `2026-09-11`**) after Telnyx rejected guest campaign **`C6NW3Y9`** twice.
