...
- **Shipped (push tap badge clear, test build 2026-05-23):** migration **`20260523180000_lounge_activity_mark_push_opened.sql`** + Edge payload **`activityEventId`** / **`activityBatchId`**; tap marks read and refreshes FAB/Alerts unread without opening Alerts panel. Apply migration + redeploy Edge on test before smoke.
