...
**Guest stake / swap / backer claim signup:** confirm email still uses **Site URL** + `/auth/confirm` (allow-list). The claim token is written onto **`user_metadata`** at signup (`poker_swap_claim_token` / `poker_stake_claim_token` / `poker_stable_claim_token`) so confirming in the IPA (a new WebView) can still attach. After confirm, attach in place and go to Bankroll / Stable ... do not bounce back to the invite page. Browser **`sessionStorage` + `localStorage`** remains a same-browser backup; a consumed-token flag stops JWT metadata from re-opening the invite URL. Optional allow-list entries for `/poker-*-claim` are not required.
