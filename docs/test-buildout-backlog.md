...
- 2026-06-09: **Lounge thread draft full media persistence (code on `test`):** migration **`20260608210000_lounge_post_drafts_thread_part_media.sql`** — root **`stream_*`** + **`thread_part_media jsonb`** (per-part images/GIF/Stream for parts 2+). Client **`loungePostDraftApi.js`** uploads all part images + persists ready Stream UIDs/posters; **`deleteLoungePostDraft`** cleans CF orphans; single-post drafts save video too. **Apply `210000` on test before smoke.** Smoke: 3-part thread with image on part 2 + video on part 3 → save draft → reopen → all media restored; delete draft → Stream orphans removed.
