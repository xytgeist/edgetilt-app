...
- 2026-05-15: **Lounge dock item order:** **`buildLoungeDockArcCarouselItems`** — **home** (wheel side anchor / L corner), then **compose → following → settings → search → chat → notifications**. Ring: clockwise from home. Edge L: bottom leg inside→out = first four; vertical leg = **search, chat, notifications**. `LoungeDockArcCarouselPrototype` still builds **`[home, …rest]`** from this array.
