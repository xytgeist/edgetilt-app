...
- 2026-06-09: **Lounge market modal Advanced pan freeze fix (2) (code on `test`):** history loader **ack** after prepend stops immediate re-fetch loop; skip history while panning; defer heavy apply off pointer-up. Opt-in pan logs: **`localStorage.setItem('loungeMarketChartPanDebug:v1','1')`** → Admin utils debug panel. Client-only.
