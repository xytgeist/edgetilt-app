...
- 2026-05-13: **Lounge OG + share polish:** Native share omits **`text`** (no duplicate iMessage bubble). **`api/lounge-post-og.js`:** **caption** → **`og:title`** (first line under image in most clients); **`og:description`** = `Name (@handle) on Edge` + **`is_og`** (U+2714) + like/comment counts when a caption exists, else stats only; **no-caption** posts use byline as title. Favicons + **`og:site_name`** for domain row. **Limitation:** OG is plain text — not Instagram’s red verified asset.
