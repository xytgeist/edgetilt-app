...
- 2026-05-15: **Lounge dock item order:** **`buildLoungeDockArcCarouselItems`** — ring / Edge L sequence **home → notifications → chat → search** (bottom leg inside→out), then **compose → following → settings** (vertical leg). `LoungeDockArcCarouselPrototype` still builds **`[home, …rest]`** from this array.
