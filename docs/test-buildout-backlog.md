...
**Test = prod:** agents must not ingest, batch-run, or patch test **`guides`** / **`machines`** unless Ryan explicitly asks. Snapshot before any approved batch: **`node scripts/ap-guide-backup-test-guides.mjs --all-published`**. See **`AGENTS.md`** **`AGENT_RULE_TEST_IS_PROD`**.
