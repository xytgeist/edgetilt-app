...
- 2026-06-07: **Fix: Lounge push broken after group-invite migration:** **`20260603190000`** replaced `activity_push_invoke_lounge_edge` (vault secrets + anon `apikey`) with `net.http_post` + unset `current_setting('app.supabase_url')` — comment/reply/mention/follow/repost/play_log/**chat_group_invite** pushes never fired. **`20260607190000_activity_events_enqueue_push_restore.sql`** restores vault invoke. Also **`441f472`**: `lounge-chat` awaits `activity_events` insert (fire-and-forget dropped on Edge return). Apply **`20260607190000`** on test + redeploy **`lounge-chat`** + **`lounge-send-activity-push`**; smoke comment/like/DM with recipient tab backgrounded.
