...
- **Live Activities:** widget target **`EdgeTiltLiveActivity`**. Real Island = background / Lock Screen. **In-app:** `EdgeLiveBankrollIslandOverlay` sized as hardware core (~126pt) + side lobes so glyph/timer/keyline clear the camera (old ~92pt pill sat under the cutout).
