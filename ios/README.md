...
| **EdgeTilt Prod** | Release | `https://edgetilt.com` … iPhone + iPad (`1,2`). Archives are a real iPad app, so App Store Connect wants iPad screenshots (12.9-inch / `APP_IPAD_PRO_3GEN_129`) before Add for Review. |
...
- **Universal Links:** Associated Domains `applinks:edgetilt.com` / `lvslotpro.com`. Paths: `/auth/confirm`, `/lounge/p/*`, `/u/*`, and SPA `/?post=` / `?u=` / `?profile=`. Share permalinks rewrite to the SPA query before WKWebView load. AASA on the live site. Safari Smart App Banner (`apple-itunes-app`, ASC **6806401093**). Gmail in-app can ignore UL; `edgetilt://auth/confirm?…` is the email fallback. New IPA for path allowlist changes. No new `EdgeNative` method.
...
- **Live Activities:** widget target **`EdgeTiltLiveActivity`**. Real Island = background / Lock Screen. In-app uses title-bar live chips (fake Island retired 2026-09-17).
