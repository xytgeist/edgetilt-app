...
- **Live Activities:** widget target **`EdgeTiltLiveActivity`**. Real Island = background / Lock Screen. **In-app:** `EdgeLiveBankrollIslandOverlay` (fake pill over WKWebView). Web `setLiveBankrollIslandOverlay` hides it on live-card bankroll screens. Tap opens manager tab only.
