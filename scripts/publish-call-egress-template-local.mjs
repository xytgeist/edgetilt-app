/**
 * Build-independent: upload dist/call-egress.html (+ logo) straight to R2 via Edge.
...
if (!fs.existsSync(htmlPath)) {
  console.error('Missing dist/call-egress.html — run: npx vite build --config vite.call-egress.config.js && node scripts/inline-call-egress-html.mjs')
  process.exit(1)
}
...
console.log('POST', endpoint, 'html_bytes=', html.length, 'logo=', path.basename(logoPath))
...
const res = await fetch(endpoint, {
  ...
  body: JSON.stringify({ html, logo_base64 }),
})
