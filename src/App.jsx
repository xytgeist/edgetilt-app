...
import {
  isAuthConfirmRecovery,
  mapAuthConfirmError,
  parseAuthConfirmFromLocation,
  routeAfterGuestClaimEmailConfirm,
  ...
  useEffect(() => {
    queueMicrotask(() => {
      void (async () => {
        ...
        const authConfirm = parseAuthConfirmFromLocation(pathname, search)
        if (authConfirm) {
          ...
          const finishAuthConfirmSuccess = async () => {
            await waitForSupabaseSession(supabase)
            if (isAuthConfirmRecovery(authConfirm)) {
              replaceUrlPreservingQuery('/reset-password')
