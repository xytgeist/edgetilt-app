...
export default function TimeWheelPicker({ value, onChange }) {
  ...
  // Emit current time on mount so parent always has a value
  useEffect(() => {
    if (!value) {
  ...
  const handleChange = (nextValue) => {
    ...
    // Carry / borrow hour when minute wraps at edges
    ...
    // Toggle AM/PM when crossing 11 ↔ 12
    if (!periodChangedByUser && prevMinuteRaw === minuteRaw) {
      if (prevHourRaw === '11' && hourRaw === '12') period = period === 'AM' ? 'PM' : 'AM'
      else if (prevHourRaw === '12' && hourRaw === '11') period = period === 'AM' ? 'PM' : 'AM'
    }
