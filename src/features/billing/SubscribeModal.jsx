...
import {
  SLOTS_EDGE_FOUNDING_MONTHLY_DURATION_MONTHS,
...
/** Interval lives on the Monthly / Annual tab, not the sticker or button. */
function ctaPriceLabel(price) {
  return String(price || '').replace(/\/(?:mo|yr)$/i, '')
}

function webFoundingThenNote(interval, listPrice) {
  const list = ctaPriceLabel(listPrice)
  if (!list) return ''
  if (interval === 'annual') return `1 yr then ${list}`
  return `${SLOTS_EDGE_FOUNDING_MONTHLY_DURATION_MONTHS} mo then ${list}`
}
  ...
  return createPortal(
    <div className="fixed inset-0 z-[210] flex items-end justify-center p-0 sm:items-center sm:p-4 sm:pt-[max(1rem,max(env(safe-area-inset-top,0px),var(--edge-sat,0px)))] sm:pb-[max(1rem,max(env(safe-area-inset-bottom,0px),var(--edge-sab,0px)))]">
      ...
      <div
        ...
        className="subscribe-modal-shell relative z-10 flex h-[89.96dvh] max-h-[89.96dvh] min-h-[89.96dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-[1.75rem] border border-zinc-700/70 bg-zinc-950 shadow-[0_24px_80px_rgba(0,0,0,0.55)] sm:h-[88.1dvh] sm:max-h-[88.1dvh] sm:min-h-[88.1dvh] sm:max-w-2xl sm:rounded-[1.75rem]"
      >
        <button
          type="button"
          onClick={onClose}
