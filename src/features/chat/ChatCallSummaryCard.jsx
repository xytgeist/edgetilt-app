 ...
 * Voice calls are history-only (no live transcript product).
 * @param {{
 *   message: {
 *     id: string,
 *     body?: string | null,
...
export function parseCallSummaryMeta(raw) {
  ...
  if (obj.kind && obj.kind !== 'call_summary') return null
  return {
    kind: 'call_summary',
    call_id: obj.call_id ? String(obj.call_id) : null,
    media_mode: obj.media_mode === 'video' ? 'video' : 'audio',
    status: obj.status ? String(obj.status) : 'ended',
    duration_seconds: Number(obj.duration_seconds) || 0,
    started_at: obj.started_at ? String(obj.started_at) : null,
    answered_at: obj.answered_at ? String(obj.answered_at) : null,
    ended_at: obj.ended_at ? String(obj.ended_at) : null,
    started_by: obj.started_by ? String(obj.started_by) : null,
...
function parseCallSummaryBody(body) {
  ...
  let status = 'ended'
  if (lower.startsWith('missed')) status = 'missed'
  else if (lower.startsWith('call declined') || lower.startsWith('declined')) status = 'declined'
  ...
  if (m) {
    ...
    durationSeconds = c != null ? a * 3600 + b * 60 + c : a * 60 + b
  }
...
function participantLabel(p) {
  if (!p) return ''
  return String(p.display_name || p.handle || '').trim()
}
