  ...
  return (
    <div className={`flex flex-wrap gap-1 ${className}`.trim()}>
      {list.map((slug) => (
        <span
          key={slug}
          className={`inline-flex max-w-full items-center truncate rounded-full border px-1.5 py-0.5 text-[10px] font-semibold leading-none tracking-tight ${loungePostCategoryPillChipClass(slug, 'display')}`}
        >
