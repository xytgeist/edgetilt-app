...
import { LOUNGE_SEARCH_SORT, writeLoungeSearchSort } from '../../utils/loungeSearchSortPref.js'
...
function labelForValue(value) {
  return OPTIONS.find((o) => o.value === value)?.label || 'Top'
}
  ...
  return (
    <div ref={wrapRef} className={`relative inline-flex ${className}`.trim()}>
      ...
      {open ? (
        <ul
          role="listbox"
          className="absolute right-0 top-full z-20 mt-0.5 min-w-[7.5rem] rounded-lg border border-zinc-700/90 bg-zinc-900 py-0.5 shadow-lg"
        >
          {OPTIONS.map((opt) => (
