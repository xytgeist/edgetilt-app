  ...
  return {
    ...
    scales: {
      ...
      y: {
        ...
        grid: {
          color: (ctx) =>
            ctx.tick.value === 0 ? 'rgba(20,20,20,0.55)' : 'rgba(255,255,255,0.05)',
...
function ChartSection({ title, data }) {
  ...
  return (
    <div
      ...
      className="rounded-3xl border border-zinc-800/60 bg-zinc-900 p-4"
    >
...
/** Poker Charts tab — same layout as slots BankrollChartsTab. */
export default function PokerBankrollChartsTab({ sessions }) {
  ...
  const yearData = useMemo(() => {
    const years = [
      ...new Set((sessions || []).map((s) => new Date(s.start_at).getFullYear())),
    ].sort((a, b) => a - b)
    if (years.length === 0) return []
    return aggregate(
      sessions,
      (s) => new Date(s.start_at).getFullYear(),
  ...
  const durationData = useMemo(() => {
    ...
    for (const s of sessions || []) {
      ...
      if (bucket) {
        map[bucket.key].total += wl
        map[bucket.key].count += 1
      }
    }
    return DURATION_BUCKETS.map((b) => ({
      label: b.label,
      total: map[b.key].total,
      count: map[b.key].count,
    }))
  }, [sessions])
  ...
  if (!sessions?.length) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <p className="text-sm text-zinc-400">No sessions yet. Complete a session to see charts.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      {yearData.length > 1 ? <ChartSection title="Year" data={yearData} /> : null}
