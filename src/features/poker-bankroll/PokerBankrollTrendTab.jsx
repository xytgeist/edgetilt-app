...
const INFO = {
  winRate: {
    title: 'Win Rate',
    body: 'The percentage of sessions that finished profitable. Cash and tournament results both count. Pair this with Payoff Ratio: a win rate under 50% can still be a strong long-run edge if your average win is larger than your average loss.',
  },
  expectancy: {
    title: 'Expectancy',
    body: 'Your average dollar result per session across the sample. Positive expectancy over a meaningful volume of hours is the foundation of a winning poker bankroll. A small positive expectancy still needs enough sessions (and bankroll) for variance to settle.',
  },
  profitFactor: {
    title: 'Profit Factor',
    body: 'Total gross winnings divided by total gross losses. Above 1.0 means the tracked sessions are profitable in aggregate. A Profit Factor of 1.4 means for every dollar lost across losing sessions, you won $1.40 across winning sessions.',
  },
  payoffRatio: {
    title: 'Payoff Ratio',
    body: 'Average winning session size divided by average losing session size. When win rate is under 50%, you generally need this above 1.0 to stay profitable. Big tournament scores and deep cash sessions push this metric up.',
  },
  stdDev: {
    title: 'Standard Deviation',
    body: 'The volatility of your session P&L in dollars. High standard deviation relative to Expectancy means larger swings per session and a larger bankroll needed to survive natural variance before your edge shows through.',
  },
  skewness: {
    title: 'Skewness',
    body: 'How asymmetric your P&L distribution is. Positive skewness means rare large wins pull the average above the median... common when tournament cashes or rare monster cash sessions drive results. Negative skewness warns that rare catastrophic losses are pulling the distribution the other way.',
  },
  sortino: {
    title: 'Sortino Ratio',
    body: 'Average session P&L divided by downside deviation only. Large wins do not count against you. For poker bankroll tracking this is a useful risk-adjusted read because you care most about limiting the losing side of variance. Higher is better; above 0 means positive risk-adjusted returns.',
  },
  calmar: {
    title: 'Calmar Ratio',
    body: 'Total net P&L divided by maximum drawdown. Measures how much return the tracked strategy generated per unit of worst-case historical risk. Above 1.0 means net profit exceeds your worst drawdown. Negative means you are still underwater overall.',
  },
  kelly: {
  ...
  maxDrawdown: {
    title: 'Max Drawdown',
    body: 'The largest peak-to-trough dollar loss across your session history. Your starting bankroll should exceed this with buffer, because future drawdowns often exceed the historical max.',
  },
  recoveryFactor: {
    title: 'Recovery Factor',
    body: 'Net profit divided by max drawdown. Shows how many times over your net gains exceed your worst historical losing stretch. Above 2.0 is generally considered strong in systematic betting contexts. Below 1.0 means net profit has not yet recovered the worst drawdown.',
  },
  ulcerIndex: {
    title: 'Ulcer Index',
    body: 'Measures both the depth and duration of drawdowns over time. Unlike Max Drawdown, which captures a single worst event, Ulcer Index penalizes long stretches spent below a high-water mark.',
  },
  riskOfRuin: {
    title: 'Risk of Ruin',
    body: 'A Monte Carlo estimate of the probability that bankroll drops to 25% of its current value within the selected projection horizon. Built from 5,000 simulations that sample your actual session history with replacement. Lower is better.',
  },
  maxWinStreak: {
    title: 'Max Win Streak',
    body: 'Your longest consecutive run of profitable sessions. Useful for calibrating expectations during hot stretches.',
  },
  maxLossStreak: {
    title: 'Max Loss Streak',
    body: 'Your longest consecutive run of losing sessions. Directly informs minimum bankroll needs: you must absorb this many sessions at typical buy-in / in-for sizes without being forced off the games you want to play.',
  },
}
...
function sessionWinLoss(s) {
  return pokerSessionWinLoss(s)
}

function sessionDurationHours(s) {
  return pokerSessionDurationHours(s)
}
    ...
    >
      <div className="w-full max-w-lg rounded-3xl bg-zinc-900 border border-zinc-700/50 px-5 pt-5 pb-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            <div className="text-white font-bold text-lg truncate">{pokerSessionStakesLabel(session)}</div>
            <div className="text-zinc-500 text-xs mt-0.5">
              {session.location_name ? `${session.location_name} · ` : ''}{fmtDate(session.start_at)}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          ...
          <DetailStat label="In for" value={fmt$(pokerSessionTotalCost(session))} />
          <DetailStat label="Cash out" value={session.cash_out != null ? fmt$(session.cash_out) : '-'} />
        </div>
        <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          <DetailStat label="Type" value={session.session_type === 'tournament' ? 'Tournament' : 'Cash'} />
        </div>
  ...
  if (sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <div className="text-zinc-400 text-sm">No sessions yet. Log sessions to see your trend.</div>
      </div>
    )
  }
