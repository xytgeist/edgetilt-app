import { IPAD_NAV_RAIL_QUERY, QUICK_LINK_MAX } from './quickLinkDestinations.js'

/**
 * Logo max-width for EDGE title bars - reserve space for right-side chrome.
 * @param {number} quickLinkCount 0–2 on phone portrait. Rail layouts (iPad + phone
 *   landscape) keep shortcuts on the left rail, so they do not shrink the wordmark.
...
export function edgeLogoTitleBarClassName(quickLinkCount, { panelCloseVisible = false, toolCloseVisible = false, liveSessionChipVisible = false } = {}) {
  ...
  const q = navRail ? 0 : Math.max(0, Math.min(QUICK_LINK_MAX, quickLinkCount))
