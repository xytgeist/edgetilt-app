...
function PullRowButton({ row, onOpenTab }) {
  return (
    <button
      type="button"
...
export function SyndicateWeeklyPullsPanel({ supabaseClient, onOpenTab }) {
  ...
  const homePcRows = board?.homePcRows || []
  const passN = [...rows, ...homePcRows].filter((r) => r.status === 'pass').length
  const failN = [...rows, ...homePcRows].filter((r) => r.status === 'fail').length

  return (
    <div className="pt-2 space-y-3" data-syndicate-weekly-pulls>
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h2 className="text-sm font-bold text-zinc-100">Data pulls</h2>
          <p className="text-[11px] text-zinc-500">
            Tuesday GHA {WEEKLY_PULL_SCHEDULE}. Home PC {HOME_PC_PULL_SCHEDULE}. Fail = failed heartbeat or overdue write.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-wrap gap-1.5 text-[11px]">
        ...
        {board?.rollup ? (
          <span className={`rounded px-2 py-0.5 font-semibold ring-1 ${BADGE[board.rollup.status] || BADGE.waiting}`}>
            Tue job {pullStatusLabel(board.rollup.status).toLowerCase()}
          </span>
        ) : null}
      </div>
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {homePcRows.map((row) => (
            <PullRowButton key={row.id} row={row} onOpenTab={onOpenTab} />
          ))}
        </div>
      </div>
      ...
      {board?.rollup ? (
        <p className="text-[11px] text-zinc-500">
          Tuesday rollup last ok {formatPullWhen(board.rollup.lastOkAt)}
