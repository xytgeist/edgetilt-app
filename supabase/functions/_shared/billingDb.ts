...
/** StoreKit purchase verified on device → same entitlements row as Stripe. */
export async function upsertAppleIapSubscription(
  ...
  args: {
    ...
    priceInterval?: 'monthly' | 'annual' | null
  ...
  const customerId = `apple_user_${userId}`
  ...
  const row = {
    ...
    status: 'active',
    current_period_end: isLifetime ? null : expiresAt,
    cancel_at_period_end: false,
    price_interval: priceInterval,
  ...
  const { data: existingByApple, error: appleLookupErr } = await admin
    ...
    .maybeSingle()
  if (appleLookupErr) {
    throw new Error(`user_subscriptions apple lookup: ${appleLookupErr.message}`)
  }
  ...
  if (existingByApple?.id) {
    const { error } = await admin.from('user_subscriptions').update(row).eq('id', existingByApple.id)
    if (error) throw new Error(`user_subscriptions apple update: ${error.message}`)
  } else {
    const { error } = await admin.from('user_subscriptions').upsert(row, {
      onConflict: 'user_id,product_slug',
    })
    if (error) throw new Error(`user_subscriptions apple upsert: ${error.message}`)
  }
