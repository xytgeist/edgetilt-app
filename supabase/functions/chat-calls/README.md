...
| `CHAT_CALL_EGRESS_USE_CUSTOM` | Set `1` to use custom pin template + `focus:<id>` layout. **Default off** (LiveKit built-in `speaker`) after custom template failed to write R2. |
