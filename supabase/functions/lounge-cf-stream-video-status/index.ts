/**
 * Read Cloudflare Stream processing status for a video uid (author polling during staged publish).
 * Caller must be authenticated. Does not verify DB ownership — uids are unguessable 32-hex values.
 *
 * Secrets: `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_STREAM_API_TOKEN` (Stream:Read or Stream:Edit).
 *
 * @see https://developers.cloudflare.com/api/resources/stream/methods/get/
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!accountId || !apiToken) {
      return new Response(
        JSON.stringify({
          error: 'Video status is not configured (missing Cloudflare Stream credentials on the server).',
        }),
    ...
    if (cfRes.status === 404) {
      return new Response(JSON.stringify({ error: 'Video not found on Cloudflare Stream.' }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (!cfRes.ok || cfJson?.success === false) {
      const msg =
        ...
        `Cloudflare Stream status failed (${cfRes.status})`
    ...
    return new Response(
      JSON.stringify({
        uid,
        state: String(status.state || '').trim() || 'unknown',
        pctComplete: String(status.pctComplete || '').trim(),
        errorReasonCode: String(status.errorReasonCode || '').trim(),
        errorReasonText: String(status.errorReasonText || '').trim(),
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
    )
  } catch (e) {
