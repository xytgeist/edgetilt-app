...
- **Slate** (`POST` with empty body): NFL only. TheRundown day events for this week plus the adjacent week (recaps + upcoming), in parallel with Odds API `/scores` (`daysFrom=3`) and live `/odds` spreads. Completed games are gone from live `/odds` ... the slate fills missing ATS from the Odds **historical** snapshot at kickoff (closing line). Other leagues are skipped on purpose ... a 6-sport sequential fetch was 502'ing the function and the Lounge painted zero pills. Vague one-team captions use live / most recent until MNF is final, then the next game. Named matchups pin that game.
