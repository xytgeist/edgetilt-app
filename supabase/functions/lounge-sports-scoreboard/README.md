...
- **Slate** (`POST` with empty body): TheRundown day events (scores, period lines, `live_game_state` when the key has it), then Odds API `/scores` (`daysFrom=3`) fills this week's completed NFL games Rundown drops after FINAL. NFL dates are **Thursday through Monday** of the current week and hold until **Tuesday 00:00 PT** (after MNF). Next week's games are filtered out.
