...
- **Detail** (`POST` `{ "event_id": "..." }`): fresh event live state, `/plays`, `/players/stats`, multi-book Odds API h2h/spreads/totals (top **8**, **Pinnacle** merged from eu-region pack and pinned first), plus pasted **`syndicate_betting_splits`** (`splits`: ticket % / handle % per side) when Action PRO / VSiN paste matches the game. Hub-open only so the 45s pill poll stays cheap. Live state includes **`possession`**, **`home_timeouts` / `away_timeouts`** (0–3 remaining) when the feed sends them (TheRundown live_game_state; ESPN scoreboard `situation` on football fallback). **PBP fallback:** when TheRundown `/plays` (or live last-play/clock) is empty, detail fills from ESPN public scoreboard/summary. **NFL** ESPN fallback is **prod only**; **CFB** (`college-football`) runs on **test + prod**. Match by abbrev, `team_id`, or soft team name.

**Live depth vs X Gametime:** Hub asks Rundown first; prod NFL/CFB hub detail can fall back to ESPN public summary for PBP/clock. Richer official/realtime still a **tier / vendor** decision (Rundown Ultra, Sportradar-class, Genius). See backlog.
