...
- **Detail** (`POST` `{ "event_id": "..." }`): fresh event live state, `/plays`, `/players/stats`, and multi-book Odds API h2h/spreads/totals (top **8**, **Pinnacle** merged from eu-region pack and pinned first). Hub-open only so the 45s pill poll stays cheap.
