-- Affiliate tax: store notification email for generated W-9/W-8 PDF copy.
-- Test only until Ryan promotes.

alter table public.affiliate_tax_profiles
  add column if not exists tax_email text;

comment on column public.affiliate_tax_profiles.tax_email is
  'Email address to receive a copy of the generated/uploaded tax attestation PDF.';
...
declare
  ...
  v_tax_email text := lower(nullif(trim(coalesce(p_payload->>'tax_email', '')), ''));
...
begin
  ...
  if v_tax_email is null or v_tax_email !~* '^[^@\s]+@[^@\s]+\.[^@\s]+$' then
    raise exception 'valid tax_email required';
  end if;
