-- Add prior-30d signup count for MoM growth on Edge Monitor New accounts card.
-- Safe re-run of 20260723240000 with users.new_prev_30d.
      ...
      'active_roster', coalesce((
        select jsonb_agg(
          jsonb_build_object(
            ...
            'stripe_subscription_id', r.stripe_subscription_id,
            'stripe_customer_id', r.stripe_customer_id,
            'creator_stripe_connect_account_id', r.creator_stripe_connect_account_id,
            'subscribed_at', r.created_at
          )
        ...
        from (
          select
            ...
            cmp.stripe_connect_account_id as creator_stripe_connect_account_id,
            cs.created_at
          ...
          left join public.creator_monetization_profiles cmp on cmp.user_id = cs.creator_user_id
...
comment on function public.admin_ops_subscriber_roster() is
  'Admin-only Edge Monitor subscriber roster: platform + creator fan subs, emails, Stripe ids, pending cancels.';
