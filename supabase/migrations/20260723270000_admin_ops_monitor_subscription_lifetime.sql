-- Edge Monitor: add subscriptions.lifetime count to snapshot (slots-edge-lifetime active/trialing).
...
comment on function public.admin_ops_monitor_snapshot() is
  'Admin-only JSON snapshot for Edge Monitor (activity by type + subscription cadence incl. lifetime).';
