-- Scott Share alert routing: lounge feed vs creator fan sub chat (+ optional lounge teaser %).
-- Apply on test before redeploying lounge-odds-poll, lounge-odds-ingest, lounge-bot-publish-due.

begin;

comment on column public.lounge_bot_odds_config.alert_audience is
  'Per alert kind destination: lounge (public feed), sub_chat (fan room only), sub_chat_10, sub_chat_30 (always sub chat + random lounge teaser).';

update public.lounge_bot_odds_config o
set alert_audience = coalesce(
  (
    ...
    from jsonb_each_text(o.alert_audience) as e(key, val)
  ),
  o.alert_audience
);

-- Patch admin save: accept new destination values + context alert keys.
...
begin
  ...
  if p_patch ? 'alert_audience' then
    ...
    for v_key, v_val in select key, value #>> '{}' from jsonb_each(p_patch->'alert_audience') loop
      ...
      if v_val not in ('lounge', 'sub_chat', 'sub_chat_10', 'sub_chat_30') then
        raise exception 'alert_audience.% must be lounge, sub_chat, sub_chat_10, or sub_chat_30', v_key;
      end if;
