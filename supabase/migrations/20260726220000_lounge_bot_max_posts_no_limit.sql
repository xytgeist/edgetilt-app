-- Bot portal: null max_posts_per_day / max_posts_per_hour = no account-level cap (news poll skips budget).

begin;

alter table public.lounge_bot_accounts
  alter column max_posts_per_day drop not null,
  alter column max_posts_per_hour drop not null;

alter table public.lounge_bot_accounts
  drop constraint if exists lounge_bot_accounts_max_posts_per_day_check,
  drop constraint if exists lounge_bot_accounts_max_posts_per_hour_check;

alter table public.lounge_bot_accounts
  add constraint lounge_bot_accounts_max_posts_per_day_check
    check (max_posts_per_day is null or max_posts_per_day between 1 and 100),
  add constraint lounge_bot_accounts_max_posts_per_hour_check
    check (max_posts_per_hour is null or max_posts_per_hour between 1 and 30);
...
begin
  ...
  if p_patch ? 'max_posts_per_day' then
    if p_patch->'max_posts_per_day' = 'null'::jsonb then
      null;
    else
      v_max_day := (p_patch->>'max_posts_per_day')::int;
      if v_max_day is null or v_max_day < 1 or v_max_day > 100 then
        raise exception 'max_posts_per_day must be 1-100 or null';
      end if;
    end if;
  end if;

  if p_patch ? 'max_posts_per_hour' then
    if p_patch->'max_posts_per_hour' = 'null'::jsonb then
      null;
    else
      v_max_hour := (p_patch->>'max_posts_per_hour')::int;
      if v_max_hour is null or v_max_hour < 1 or v_max_hour > 30 then
        raise exception 'max_posts_per_hour must be 1-30 or null';
      end if;
    end if;
  end if;
  ...
  set
    ...
    max_posts_per_day = case
      when not (p_patch ? 'max_posts_per_day') then max_posts_per_day
      when p_patch->'max_posts_per_day' = 'null'::jsonb then null
      else (p_patch->>'max_posts_per_day')::int
    end,
    max_posts_per_hour = case
      when not (p_patch ? 'max_posts_per_hour') then max_posts_per_hour
      when p_patch->'max_posts_per_hour' = 'null'::jsonb then null
      else (p_patch->>'max_posts_per_hour')::int
    end,
...
comment on column public.lounge_bot_accounts.max_posts_per_day is
  'Account-level daily post cap (news bots). Null = no limit. Scott uses per-alert caps in lounge_bot_odds_config.';

comment on column public.lounge_bot_accounts.max_posts_per_hour is
  'Account-level hourly post cap (news bots). Null = no limit.';
