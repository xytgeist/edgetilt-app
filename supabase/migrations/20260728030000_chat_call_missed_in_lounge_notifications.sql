-- Show chat_call_missed in Lounge in-app notifications (badge + list + tap → Call back).
-- Keep chat_call_invite / chat_dm / chat_group_invite push-routing only.
-- Also restore play_log_* columns dropped by later CREATE OR REPLACE / DROP recreates.
...
comment on function public.lounge_activity_events_page(integer, timestamptz, uuid) is
  'Lounge notifications page. Includes chat_call_missed (tap → chat missedCall=). Excludes chat_dm / group invite / ringing invite.';
