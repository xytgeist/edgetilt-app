-- Perf fix: admin_ops_system_health_snapshot() timed out scanning cron.job_run_details
-- (lounge_activity_push_flush every 10s → millions of rows). Latest run per job only,
-- scoped to registry jobnames + 3-day window.
...
begin
  ...
  registry_jobnames as (
    select distinct r.jobname
    from registry r
    where r.jobname is not null
  ),
  latest_cron_runs as (
    ...
    from cron.job j
    inner join registry_jobnames rj on rj.jobname = j.jobname
    inner join cron.job_run_details d on d.jobid = j.jobid
    where d.start_time >= v_now - interval '3 days'
  ...
  job_rows as (
      ...
      case
        ...
        when lr.last_start < v_now - make_interval(mins => greatest(r.max_stale_minutes, 1))
          then format(
            ...
            to_char(v_now - lr.last_start, 'FMHH24"h "FMMI"m" ago"'),
            r.max_stale_minutes
          )
        else null
      end as hint
    ...
    left join latest_cron_runs lr on lr.jobid = j.jobid
  )
  select
...
comment on function public.admin_ops_system_health_snapshot() is
  'Admin Edge Monitor: pg_cron job health + billing drift. Perf: 3-day cron window + registry-scoped jobids.';
