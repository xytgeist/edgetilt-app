-- System health perf v4: do NOT scan cron.job_run_details (millions of push-flush rows).
-- cron.job active flag + domain heartbeat signals only.
...
comment on function public.admin_ops_scheduled_jobs_snapshot() is
  'Admin Edge Monitor: pg_cron registry (cron.job) + optional domain heartbeats. Skips cron.job_run_details.';
