-- Edge Monitor: activity push flush stale only when overdue pending batches exist.
-- Quiet prod (no likes/bookmarks) no longer false-alarms as stale.
...
comment on function public.admin_ops_scheduled_jobs_snapshot() is
  'Admin Edge Monitor: pg_cron registry + domain heartbeats. Push flush stale = overdue activity_push_batches only.';
