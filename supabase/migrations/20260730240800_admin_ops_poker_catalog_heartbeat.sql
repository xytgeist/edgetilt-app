-- External job heartbeats for Edge Monitor (GitHub Actions, etc.).
-- Service role writes via admin_ops_record_job_heartbeat; admins read via scheduled_jobs_snapshot.

create table if not exists public.admin_ops_job_heartbeats (
...
comment on table public.admin_ops_job_heartbeats is
  'Last success/failure timestamps for external scheduled jobs (e.g. GitHub Actions poker catalog sync).';

alter table public.admin_ops_job_heartbeats enable row level security;

create or replace function public.admin_ops_record_job_heartbeat(
  p_job_id text,
  p_status text,
...
as $$
declare
  v_job_id text := btrim(coalesce(p_job_id, ''));
  v_status text := lower(btrim(coalesce(p_status, 'unknown')));
begin
  if v_job_id = '' then
    raise exception 'job_id required';
  end if;

  insert into public.admin_ops_job_heartbeats as h (
    job_id,
    last_success_at,
    last_failure_at,
    last_status,
  ...
  values (
    v_job_id,
    case when v_status = 'ok' then now() else null end,
  ...
  on conflict (job_id) do update set
    last_success_at = case
      when excluded.last_status = 'ok' then excluded.last_success_at
      else h.last_success_at
    end,
    last_failure_at = case
      when excluded.last_status = 'failed' then excluded.last_failure_at
...
revoke all on function public.admin_ops_record_job_heartbeat(text, text, jsonb) from public;
grant execute on function public.admin_ops_record_job_heartbeat(text, text, jsonb) to service_role;

comment on function public.admin_ops_record_job_heartbeat(text, text, jsonb) is
  'Record external job heartbeat (GitHub Actions). service_role only.';
  ...
  loop
    ...
    if coalesce(r.kind, 'pg_cron') = 'external' then
      if coalesce(r.heartbeat, '') like 'ops_job:%' then
        ...
        select h.last_success_at, h.last_status, h.last_detail
        into v_ops_job_success, v_ops_job_status, v_ops_job_detail
        from public.admin_ops_job_heartbeats h
        where h.job_id = v_ops_job_id;

        v_last_start := v_ops_job_success;
        if v_ops_job_success is null then
          v_health := 'stale';
          ...
          if v_ops_job_detail ? 'rows' then
            v_hint := v_hint || format(' Rows: %s.', v_ops_job_detail->>'rows');
          end if;
        end if;
      else
        v_health := 'external';
...
comment on function public.admin_ops_scheduled_jobs_snapshot() is
  'Admin Edge Monitor: pg_cron + domain heartbeats + admin_ops_job_heartbeats for GitHub Actions.';
