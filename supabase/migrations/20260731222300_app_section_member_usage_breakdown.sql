-- Per-member section / calculator / session breakdown for top-N list + handle lookup.
...
comment on function public.app_product_analytics_member_breakdown(uuid, timestamptz, timestamptz) is
  'Internal: per-member tab visits, calculator opens, and session saves by section (24h + 7d).';
...
comment on function public.admin_ops_app_section_member_usage_snapshot(text, int) is
  'Admin-only: top active members (7d) with per-section breakdown; optional handle lookup.';
