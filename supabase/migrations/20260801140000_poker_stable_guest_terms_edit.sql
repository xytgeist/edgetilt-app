-- Guest-only stake terms: stakee may edit active guest deals; reassign guest slice → Edge user invite.

create or replace function public.poker_stable_deal_is_guest_only(p_deal_id uuid)
returns boolean
language sql
stable
security definer
...
begin
  ...
  if jsonb_array_length(p_slices) = 0 and v_status = 'pending' then
...
create or replace function public.poker_stable_reassign_guest_slice(
  p_slice_id uuid,
  p_staker_user_id uuid
)
returns void
language plpgsql
security definer
...
begin
  ...
  if p_staker_user_id is null then
    raise exception 'Pick an Edge user';
  end if;

  select s.deal_id, d.stakee_user_id
  into v_deal_id, v_stakee
  from public.poker_stable_deal_slices s
  ...
  if v_deal_id is null then
    raise exception 'You cannot reassign this guest backer';
  end if;

  if p_staker_user_id = v_stakee then
    raise exception 'You cannot add yourself as a backer';
  end if;
  ...
  if exists (
    ...
    where s.deal_id = v_deal_id
      and s.staker_user_id = p_staker_user_id
  ) then
    raise exception 'That Edge user already has a slice on this deal';
  end if;

  update public.poker_stable_deal_slices
  set
    counterparty_kind = 'user',
    staker_user_id = p_staker_user_id,
    guest_label = null,
    guest_phone = null,
    guest_email = null,
    status = 'pending',
...
revoke all on function public.poker_stable_deal_is_guest_only(uuid) from public;
grant execute on function public.poker_stable_deal_is_guest_only(uuid) to authenticated;

revoke all on function public.poker_stable_apply_stakee_terms(uuid, jsonb, jsonb, boolean) from public;
grant execute on function public.poker_stable_apply_stakee_terms(uuid, jsonb, jsonb, boolean) to authenticated;

revoke all on function public.poker_stable_reassign_guest_slice(uuid, uuid) from public;
grant execute on function public.poker_stable_reassign_guest_slice(uuid, uuid) to authenticated;
