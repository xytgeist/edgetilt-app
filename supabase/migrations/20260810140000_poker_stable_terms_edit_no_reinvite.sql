-- Pending terms edit was delete+reinsert slices → AFTER INSERT slice_invite fired again
-- (Edge Lord got a brand-new invite toast/push). Suppress invite during apply; emit
-- poker_stable_terms_edited once per pending Edge backer instead.
...
comment on constraint activity_events_event_type_check on public.activity_events is
  'Allowed activity_events.event_type values (includes poker_stable_terms_edited).';
...
declare
  ...
  v_detail text;
begin
  -- Terms edit / other bulk slice rebuilds set this for the transaction.
  if coalesce(current_setting('poker_stable.suppress_slice_invite', true), '') = '1' then
    return new;
  end if;
...
begin
  ...
  if v_status in ('pending', 'revoked') then
    -- Prevent delete+reinsert from looking like a brand-new syndicate invite.
...
comment on function public.poker_stable_apply_stakee_terms(uuid, jsonb, jsonb, boolean) is
  'Stakee applies deal/slice terms; pending rebuild suppresses slice_invite and emits poker_stable_terms_edited.';
