-- Player tournament close Overall P/L includes earned markup kept
-- (prepaid fee − unused refund), not just returned − contribution.
...
begin
  ...
  if p_finalize and v_deal.deal_type = 'tournament_package' then
    if p_user_id = v_stakee then
      ...
      v_stake_pl := public.poker_stable_round_money(
        v_return - public.poker_stable_round_money(v_baseline * (v_retained / 100.0))
      );

      select public.poker_stable_round_money(coalesce(sum(a.fee_amount), 0))
      into v_fee_total
      from public.poker_stable_deal_slices s
      join public.poker_stable_backer_allocations a on a.slice_id = s.id
      where s.deal_id = v_deal.id
        and s.status not in ('declined', 'cancelled')
        and coalesce(a.fee_posted, false);

      v_buyins := public.poker_stable_deal_tournament_buyins(v_deal.id);
      if v_fee_total > 0.005 and v_baseline > 0.005 then
        v_applied := public.poker_stable_round_money(
          v_fee_total * (least(v_buyins, v_baseline) / v_baseline)
        );
      else
        v_applied := 0;
      end if;
      v_overall := public.poker_stable_round_money(v_stake_pl + v_applied);

      if v_applied > 0.005 then
        v_player_msg := format(
          '%s: %s returned to your personal bankroll (your share of the %s remaining stake). Overall P/L %s%s (stake %s%s · markup +%s).',
          v_prefix,
          ...
          case when v_stake_pl > 0 then '+' else '' end,
          public.poker_stable_fmt_money(v_stake_pl),
          public.poker_stable_fmt_money(v_applied)
        );
      else
        v_player_msg := format(
          '%s: %s returned to your personal bankroll (your share of the %s remaining stake). Overall P/L %s%s.',
          v_prefix,
          public.poker_stable_fmt_money(v_return),
          public.poker_stable_fmt_money(v_roll),
          case when v_overall > 0 then '+' else '' end,
...
-- Backfill player tournament close rows that omit markup from Overall P/L.
do $$
declare
  r record;
  v_deal public.poker_stable_deals%rowtype;
...
begin
  for r in
    select le.id, le.deal_id, le.settlement_id, le.user_id
    ...
    where le.entry_kind = 'close_settlement'
      ...
      and le.user_id = d.stakee_user_id
      and le.message ilike '%Overall P/L%'
      and le.message not ilike '%markup +%'
  loop
    ...
    if v_applied > 0.005 then
      v_msg := format(
        'Close settlement: %s returned to your personal bankroll (your share of the %s remaining stake). Overall P/L %s%s (stake %s%s · markup +%s).',
        public.poker_stable_fmt_money(v_return),
        public.poker_stable_fmt_money(v_roll),
        case when v_overall > 0 then '+' else '' end,
        public.poker_stable_fmt_money(v_overall),
      ...
      v_msg := format(
        'Close settlement: %s returned to your personal bankroll (your share of the %s remaining stake). Overall P/L %s%s.',
        public.poker_stable_fmt_money(v_return),
        public.poker_stable_fmt_money(v_roll),
        case when v_overall > 0 then '+' else '' end,
        public.poker_stable_fmt_money(v_overall)
