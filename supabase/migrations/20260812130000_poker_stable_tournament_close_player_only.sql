-- Tournament package close: only the Edge player may finalize.
-- Multi-backer packages must not let any one backer close for everyone.
-- Guest-only packages (no stakee) keep backer close.
...
begin
  ...
  if p_finalize
     and v_deal.deal_type = 'tournament_package'
     and v_deal.stakee_user_id is not null
     and v_deal.stakee_user_id <> v_uid then
    raise exception 'Only the player can close a tournament package stake';
  end if;
