-- Default swap math: extra bullets at face (Ryan examples 2026-08-14).
-- Apply on TEST first. Do not apply to production without Ryan's explicit ask.

alter table public.poker_tournament_swaps
  add column if not exists creator_bullets integer,
  add column if not exists counterparty_bullets integer;

comment on column public.poker_tournament_swaps.creator_bullets is
  '1 + re-entries. Used for extra-bullet face vs the other side.';
comment on column public.poker_tournament_swaps.counterparty_bullets is
  '1 + re-entries. Used for extra-bullet face vs the other side.';
...
begin
  ...
  if v_creator_cashed then
    creator_net := coalesce(s.creator_prize, 0) - v_face - v_face_c - v_creator_pays_face;
  ...
  if v_cp_cashed then
    counterparty_net := coalesce(s.counterparty_prize, 0) - v_face - v_face_p - v_cp_pays_face;
