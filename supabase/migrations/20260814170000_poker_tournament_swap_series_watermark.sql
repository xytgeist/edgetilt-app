-- Multi-flight series: current-bullet-forward watermark.
-- creator_bullets / counterparty_bullets store the series Day-1 total.
-- exclude_prior skips already-fired bullets unless Include previous is on.
-- Apply on TEST first.

alter table public.poker_tournament_swaps
  add column if not exists include_previous_bullets boolean not null default false,
  add column if not exists creator_exclude_prior_bullets integer not null default 0,
  add column if not exists counterparty_exclude_prior_bullets integer not null default 0;

comment on column public.poker_tournament_swaps.include_previous_bullets is
  'Creator opted to count already-fired series bullets. Default is current bullet forward.';
comment on column public.poker_tournament_swaps.creator_exclude_prior_bullets is
  'Series bullets already fired before this swap (skipped unless include_previous_bullets).';
comment on column public.poker_tournament_swaps.counterparty_exclude_prior_bullets is
  'Counterparty series bullets already fired before they accepted.';
