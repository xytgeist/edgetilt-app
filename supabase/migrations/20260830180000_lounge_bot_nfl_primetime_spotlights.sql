-- ============================================================================
-- lounge_bot_nfl_primetime_spotlights: Automated TNF / SNF / MNF Deep Dives
-- 1. Updates invoke_lounge_odds_poll to allow 'nfl_primetime_spotlight'
-- 2. Schedules automated pg_cron jobs for:
--    - TNF: Thursdays @ 3:30 PM PT (22:30 UTC)
--    - SNF: Sundays @ 3:30 PM PT (22:30 UTC)
--    - MNF: Mondays @ 3:30 PM PT (22:30 UTC)
-- ============================================================================
...
begin
  ...
  execute $func$
    ...
    begin
      ...
      if action not in ('poll_edges', 'poll_live', 'daily_slates', 'best_bet_hour', 'value_bet_radar', 'grade_picks', 'predictive_pick', 'nfl_slate_card', 'nfl_wong_teaser', 'nfl_primetime_spotlight', 'calibrate_persona_models') then
  ...
  -- 2. TNF Spotlight: Thursdays @ 3:30 PM PT (22:30 UTC) - Day 4
  if exists (select 1 from cron.job where jobname = 'lounge_odds_nfl_tnf_spotlight') then
    perform cron.unschedule('lounge_odds_nfl_tnf_spotlight');
  end if;
  perform cron.schedule(
    'lounge_odds_nfl_tnf_spotlight',
    '30 22 * * 4',
    'select public.invoke_lounge_odds_poll(''nfl_primetime_spotlight'');'
  );

  -- 3. SNF Spotlight: Sundays @ 3:30 PM PT (22:30 UTC) - Day 0
  if exists (select 1 from cron.job where jobname = 'lounge_odds_nfl_snf_spotlight') then
    perform cron.unschedule('lounge_odds_nfl_snf_spotlight');
  end if;
  perform cron.schedule(
    'lounge_odds_nfl_snf_spotlight',
    '30 22 * * 0',
    'select public.invoke_lounge_odds_poll(''nfl_primetime_spotlight'');'
  );

  -- 4. MNF Spotlight: Mondays @ 3:30 PM PT (22:30 UTC) - Day 1
