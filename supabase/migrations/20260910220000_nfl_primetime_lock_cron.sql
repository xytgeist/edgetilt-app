-- Primetime 90-min inactives lock. Desk action + Thu/Sun/Mon poll window.
-- Edge still no-ops outside 70-110 min pre-kick unless Ops Preview/Publish.
...
-- PDT (UTC-7): 2-4:50pm PT on Sun/Mon/Thu, then 5-8:50pm PT (UTC next civil day).
do $$
begin
  if exists (select 1 from cron.job where jobname = 'lounge_odds_nfl_primetime_lock_afternoon') then
    perform cron.unschedule('lounge_odds_nfl_primetime_lock_afternoon');
  end if;
  perform cron.schedule(
    'lounge_odds_nfl_primetime_lock_afternoon',
    '*/10 21-23 * * 0,1,4',
    'select public.invoke_lounge_odds_poll(''nfl_primetime_lock'');'
  );

  if exists (select 1 from cron.job where jobname = 'lounge_odds_nfl_primetime_lock_evening') then
    perform cron.unschedule('lounge_odds_nfl_primetime_lock_evening');
  end if;
  perform cron.schedule(
    'lounge_odds_nfl_primetime_lock_evening',
    '*/10 0-3 * * 1,2,5',
    'select public.invoke_lounge_odds_poll(''nfl_primetime_lock'');'
  );
end $$;
