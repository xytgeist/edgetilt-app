-- Edge Monitor: register home-PC Action public-betting + ESPN trench syncs.
-- Heartbeats via admin_ops_record_job_heartbeat from Windows Task Scheduler.
  ...
  for r in
    ...
    from jsonb_to_recordset(
      '[
        ...
        {"id":"syndicate_football_metrics_sync_production","jobname":null,"label":"Syndicate NFL/CFB/UFC metrics sync","category":"bots","kind":"external","schedule_hint":"GitHub Actions · Tue 14:00 UTC (~7am PT)","max_stale_minutes":10080,"critical":true,"runbook_id":"lounge-bot-sports-odds","heartbeat":"ops_job:syndicate_football_metrics_sync_production"},
        {"id":"syndicate_action_public_betting_sync_production","jobname":null,"label":"Action Network public betting sync","category":"bots","kind":"external","schedule_hint":"Home PC · daily 10am + 6pm PT","max_stale_minutes":1560,"critical":true,"runbook_id":"lounge-bot-sports-odds","heartbeat":"ops_job:syndicate_action_public_betting_sync_production"},
        {"id":"syndicate_espn_nfl_trench_sync_production","jobname":null,"label":"ESPN NFL trench win rates sync","category":"bots","kind":"external","schedule_hint":"Home PC · daily 10am + 6pm PT","max_stale_minutes":1560,"critical":true,"runbook_id":"lounge-bot-sports-odds","heartbeat":"ops_job:syndicate_espn_nfl_trench_sync_production"}
      ]'::jsonb
    ) as x(
    ...
    if coalesce(r.kind, 'pg_cron') = 'external' then
      if coalesce(r.heartbeat, '') like 'ops_job:%' then
        ...
        if v_ops_job_status = 'failed' then
          ...
          v_health := 'stale';
          v_hint := format(
            'Last success %s UTC — expected on schedule (%s).',
            to_char(v_ops_job_success at time zone 'UTC', 'YYYY-MM-DD HH24:MI:SS'),
            coalesce(nullif(btrim(r.schedule_hint), ''), 'external')
          );
        else
          v_health := 'ok';
...
comment on function public.admin_ops_scheduled_jobs_snapshot() is
  'Admin Edge Monitor: pg_cron + external heartbeats (poker catalog, syndicate GHA metrics, home-PC Action splits + ESPN trench).';
